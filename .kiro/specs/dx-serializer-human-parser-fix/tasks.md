...
- [ ] 1. Fix key-value line parsing in humanParserV3.ts
  - [ ] 1.1 Update `parseKeyValueLineV3` to add validation for values starting with `=`
    - Add warning log when value starts with `=` (indicates upstream issue)
    - Ensure proper trimming of both key and value
    - _Requirements: 2.1, 2.2, 2.3_

  - [ ] 1.2 Write property test for key-value parsing correctness
    - **Property 2: Key-Value Parsing Correctness**
    - **Validates: Requirements 1.1, 1.4, 2.1, 5.1**

- [ ] 2. Fix stack section parsing in humanParserV3.ts
  - [ ] 2.1 Update stack section handling in `parseHumanV3` function
