We are creating dx-www that will beat every current frameworks of this time in every possible way - We already defeated svelt in his speed - So, please list all other thing we have beat as current we didn't beated axtic web yet - so please list all best feats of current best web frameworks but remove some useless features feats like "Fastify" as its best feat is about its connectivity of nodejs it is not a huge acheivement and its only a acheivement only when you are using nodejs - so please remove these types of useless feats and then verify it with current dx-www so that I can know what to do next as its 17 Decemeber 2025 and I have to release dx-www at 1st January!

Here is the list of current best frameworks:
```markdown
