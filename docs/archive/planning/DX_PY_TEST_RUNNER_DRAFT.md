I already created the fastest js runtime by using dx binary dawn game changing features now I want to make dx-py-test-runner and it to be the fastest runner in python - So, please give me at least 10 game changinge plan to create dx-py-test-runner to be at least 10x faster than unittest in python.

Here is more about current fastest test runner:
```markdown
## 🏆 The Current Fastest Python Test Runners

| Runner | Why it's "Fast" | The Trade-off |
| --- | --- | --- |
| **`unittest`** (Standard) | Zero external dependencies; minimal overhead. It is the "pure speed" baseline for simple suites. | Missing modern DX (fixtures, auto-discovery is clunky, output is plain). |
| **`Ward`** | Modern, built from the ground up for speed with a simplified discovery engine and zero-config parallelization. | Smaller ecosystem than Pytest; less plugin support. |
| **`Maelstrom`** | A specialized runner that uses **Rust** for its core orchestration and runs tests in isolated containers for extreme parallelization. | Requires more setup (often used in monorepos or cloud-scale CI). |
| **`Mamba`** | Fast, BDD-style runner. It utilizes a custom execution engine that avoids some of the heavy setup/teardown overhead found in Pytest. | Syntax is specific (Gherkin-like), which isn't for everyone. |
| **`Pytest`** (Optimized) | While slow out of the box, when paired with **`pytest-xdist`** or **`pytest-parallel`**, it is the industry standard for throughput. | **Massive startup overhead** and slow discovery on large codebases. |

---

## 🛠️ The "Speed Blueprint" for `dx-python-test-runner`

If you want `dx-python-test-runner` to be the "Bun" or "Vitest" of the Python world, you should target these three bottlenecks where traditional runners fail:

### 1. Test Discovery (The Rust Advantage)

Standard runners (like Pytest) walk your file tree and import files to find tests. This is incredibly slow on large projects.

* **The Hack:** Use a **compiled core (Rust/C++)** for file-system crawling. Only import a file when you are *certain* it contains a test. Tools like `ruff` are fast because they parse the AST (Abstract Syntax Tree) without actually executing the Python code.

### 2. The Startup "Cold Boot" Problem

Every time you run a test, Python has to re-initialize and re-import all your dependencies (Django, SQLAlchemy, etc.).

* **The Hack:** Implement a **Daemon Mode** or **Warm Worker Pool**. Keep a background process alive with pre-loaded modules so that running a test feels like an instant function call rather than a full process boot.

### 3. Smart Change-Detection (HMR for Tests)

Running 1,000 tests when you only changed one line in one file is a waste of time.

* **The Hack:** Build a dependency graph of your project. When a file changes, `dx-python-test-runner` should only run the tests that directly or indirectly import that specific file.

---

## 📉 Visualizing the Performance Gap

> **Pro-Tip:** If you really want to win, look into **Bytecode Injection**. Some experimental runners bypass the standard `import` system to inject test code directly into the VM, saving milliseconds on every file load.

**Would you like me to help you draft the architectural requirements or a basic Rust-based file discovery script for your runner?**
```


And here is more dx-js-test-runner and how we become best:
```markdown
