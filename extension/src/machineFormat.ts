...
import { DxDocument, DxValue, DxSection, createDocument, createSection, strValue, numValue, boolValue, nullValue, arrValue } from './llmParser';
...
export function machineToDocument(machine: MachineDocument): DxDocument {
    ...
    for (const [id, section] of Object.entries(machine.sections)) {
        const dxSection = createSection(id, section.schema);
...
export function serializeToBinary(doc: DxDocument): Uint8Array {
    ...
    for (const [id, section] of doc.sections) {
        ...
        for (const col of section.schema) writer.writeShortString(col);
