...
mod property_tests {
    ...
    use crate::io::{create_async_io, blocking::BlockingIO, AsyncFileIO};
    ...
    proptest! {
        ...
        fn prop_batch_write_equals_individual_writes(
            ...
            let mut batch_files: Vec<(&Path, &[u8])> = Vec::new();
            for (i, content) in file_contents.iter().enumerate() {
    ...
    fn test_round_trip_through_batch_operations() {
        ...
        // Write in batch
        let write_files: Vec<(&Path, &[u8])> = files_data.iter()
            .map(|(p, d)| (p.as_path(), d.as_slice()))
            .collect();
        io.write_batch_sync(&write_files).unwrap();
        
        // Read in batch
        let read_paths: Vec<&Path> = files_data.iter()
            .map(|(p, _)| p.as_path())
            .collect();
