...
//! ```text
//! project/
//!   config.dx          -> .dx/serializer/a7f3c2e1/config.dxs
//!   src/config.dx      -> .dx/serializer/b8d4f1a2/config.dxs
//!   lib/config.dx      -> .dx/serializer/c9e5g3b4/config.dxs
//! ```
//!
...
/// Get the output path for a .dxs file
///
/// # Arguments
...
///
/// # Returns
/// The full path where the .dxs file should be written
///
/// # Example
/// ```ignore
/// let config = BinaryConfig::with_root("/project");
/// let output = get_binary_path("/project/src/config.dx", &config);
/// // Returns: /project/.dx/serializer/a7f3c2e1/config.dxs
/// ```
pub fn get_binary_path<P: AsRef<Path>>(source_path: P, config: &BinaryConfig) -> PathBuf {
    ...
    config.output_dir.join(&hash).join(format!("{}.dxs", filename))
}
...
/// Read binary data from .dxs file
...
/// Get manifest of all .dxs files in a project
    ...
    for entry in fs::read_dir(&config.output_dir)? {
        ...
        if path.is_dir() {
            ...
            for file_entry in fs::read_dir(&path)? {
                ...
                if file_path.extension().map(|e| e == "dxs").unwrap_or(false) {
                    if let Some(h) = &hash {
...
mod tests {
    ...
    fn test_get_binary_path() {
        ...
        assert!(path.to_string_lossy().ends_with(".dxs"));
    }
    ...
    fn test_binary_path_different_dirs() {
        ...
        assert!(path1.file_name() == path2.file_name()); // Both are config.dxs
    }
}
