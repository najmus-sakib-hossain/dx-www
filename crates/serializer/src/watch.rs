//! File Watcher for .dxs and dx config files
//!
//! Monitors filesystem for changes to:
//! - Root `dx` config file (no extension)
//! - All `.dxs` rule definition files
//!
...
pub enum FileChange {
    ...
    /// A .dxs rule file changed
    RuleFileChanged(PathBuf),
    /// A .dxs rule file was created
    RuleFileCreated(PathBuf),
    /// A .dxs rule file was deleted
    RuleFileDeleted(PathBuf),
}
/// File watcher for dx config and .dxs files
#[cfg(feature = "watch")]
...
impl DxWatcher {
    ...
    fn process_event(event: Event) -> Option<FileChange> {
        ...
        // Check if it's a .dxs file
        if path.extension()?.to_str()? == "dxs" {
            return Some(match event.kind {
    ...
    /// Watch a directory for dx and .dxs files
    ///
...
/// Helper function to find all .dxs files in a directory
#[cfg(feature = "watch")]
...
mod tests {
    ...
    fn test_find_dxs_files() {
        ...
        // Create some .dxs files
        fs::write(rules_dir.join("js-rules.dxs"), "# JS rules").unwrap();
        fs::write(rules_dir.join("py-rules.dxs"), "# Python rules").unwrap();
