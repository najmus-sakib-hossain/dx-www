...
impl DxMmap {
    ...
    /// Validate the DX-Zero header
    pub fn validate_header(&self) -> Result<()> {
        let bytes = self.as_bytes();
        if bytes.len() < 4 {
            return Err(super::types::DxZeroError::BufferTooSmall {
                required: 4,
                actual: bytes.len(),
            });
        }
        ...
        if bytes[0] != 0x5A || bytes[1] != 0x44 {
            return Err(super::types::DxZeroError::InvalidMagic);
        }

        // Check version
        if bytes[2] != 0x01 {
            return Err(super::types::DxZeroError::UnsupportedVersion {
                found: bytes[2],
                supported: 0x01,
            });
        }
...
impl<'a> DxMmapBatch<'a> {
    ...
    pub fn get(&self, index: usize) -> Option<QuantumReader<'_>> {
        ...
        if end <= self.mmap.len() {
            Some(QuantumReader::new(
                &self.mmap.as_bytes()[offset..end],
            ))
        } else {
            None
        }
