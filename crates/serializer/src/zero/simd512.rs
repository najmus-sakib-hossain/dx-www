    ...
    pub fn find_byte(haystack: &[u8], needle: u8) -> Option<usize> {
        #[cfg(all(target_arch = "x86_64", target_feature = "avx512f", target_feature = "avx512bw"))]
        {
            if haystack.len() >= 64 {
                ...
                for i in 0..chunks {
                    let offset = i * 64;
                    if let Some(pos) = unsafe { super::avx512::find_byte_64(&haystack[offset..], needle) } {
...
/// Batch operations for DX-Zero records
pub mod batch {
    use super::dispatch;
    ...
    #[inline]
    pub fn sum_field_u64(data: &[u8], field_offset: usize, record_size: usize, count: usize) -> u64 {
    ...
    #[inline]
    pub fn sum_field_u32(data: &[u8], field_offset: usize, record_size: usize, count: usize) -> u64 {
