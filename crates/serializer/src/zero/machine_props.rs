//! Property tests for Machine format (DX-Zero)
//!
...
mod property_tests {
    ...
    use crate::zero::slot::{DxZeroSlot, MAX_INLINE_SIZE, INLINE_MARKER, HEAP_MARKER};
    use crate::zero::mmap::{DxMmap, DxMmapBatch};
    use crate::zero::compress::{DxCompressed, CompressionLevel, StreamCompressor, StreamDecompressor};
    use crate::zero::simd512::{dispatch, portable};
    ...
    proptest! {
        ...
        fn prop_string_storage_inline_threshold(s in "[ -~]{0,30}") {
            ...
            if bytes.len() <= MAX_INLINE_SIZE {
                ...
                let slot = DxZeroSlot::inline_from_bytes(bytes).unwrap();
                ...
                let slot = DxZeroSlot::heap_reference(0, bytes.len() as u32);
        ...
        fn prop_heap_reference_stores_correctly(offset in 0u32..u32::MAX, length in 0u32..u32::MAX) {
            let slot = DxZeroSlot::heap_reference(offset, length);
            prop_assert!(slot.is_heap());
    ...
    fn test_slot_boundary_15_bytes_fails() {
        ...
        let result = DxZeroSlot::inline_from_bytes(data);
