...
use crate::llm::convert::{document_to_llm, document_to_machine, ConvertError};
...
impl CacheGenerator {
    ...
    pub fn map_path_to_cache(&self, source_path: &Path, base_path: &Path) -> CachePaths {
        ...
        // Build cache paths
        let llm_path = self.config.cache_root
            .join("llm")
            .join(&relative_str)
            .with_extension("dx.llm");

        let machine_path = self.config.cache_root
            .join("machine")
            .join(&relative_str)
    ...
    pub fn generate(&self, source_path: &Path, base_path: &Path) -> Result<CacheResult, CacheError> {
        // Read source file
    ...
    fn write_cache_file(&self, path: &Path, content: &[u8]) -> Result<(), CacheError> {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| CacheError::DirectoryCreation(format!("{}: {}", parent.display(), e)))?;
        }
