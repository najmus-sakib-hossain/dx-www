...
mod property_tests {
    ...
    fn test_special_values_through_all_formats() {
        let mut doc = DxDocument::new();
        doc.context
            .insert("flag_true".to_string(), DxLlmValue::Bool(true));
        doc.context
            .insert("flag_false".to_string(), DxLlmValue::Bool(false));
        doc.context
            .insert("empty".to_string(), DxLlmValue::Null);
    ...
    fn test_reference_resolution_in_human() {
        let mut doc = DxDocument::new();
        doc.refs
            .insert("A".to_string(), "Shared Value Here".to_string());
        ...
        let human = llm_to_human(&llm).unwrap();
        assert!(
            human.contains("Shared Value Here"),
            "Human should contain resolved value"
        );
    }
    ...
    fn test_machine_format_preserves_all_types() {
        let mut doc = DxDocument::new();
        doc.context
            .insert("str".to_string(), DxLlmValue::Str("Hello".to_string()));
        doc.context.insert("num".to_string(), DxLlmValue::Num(3.14));
        doc.context
            .insert("bool_t".to_string(), DxLlmValue::Bool(true));
        doc.context
            .insert("bool_f".to_string(), DxLlmValue::Bool(false));
        ...
        doc.refs
            .insert("R".to_string(), "Reference Value".to_string());

        let machine = document_to_machine(&doc);
        let round_trip = machine_to_document(&machine).unwrap();

        assert_eq!(
            round_trip.context.get("str").unwrap().as_str(),
            Some("Hello")
        );
        assert!((round_trip.context.get("num").unwrap().as_num().unwrap() - 3.14).abs() < 0.001);
        assert_eq!(
            round_trip.context.get("bool_t").unwrap().as_bool(),
            Some(true)
        );
        assert_eq!(
            round_trip.context.get("bool_f").unwrap().as_bool(),
            Some(false)
        );
