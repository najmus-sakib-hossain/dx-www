...
mod property_tests {
    ...
    fn test_human_special_values_round_trip() {
        ...
        let mut doc = DxDocument::new();
        let mut section = DxSection::new(vec!["id".to_string(), "flag".to_string(), "empty".to_string()]);
        section.add_row(vec![
            DxLlmValue::Num(1.0),
    ...
    fn test_human_unicode_table_format() {
        ...
        section.add_row(vec![DxLlmValue::Num(1.0), DxLlmValue::Str("Test".to_string())]).unwrap();
        doc.sections.insert('d', section);
