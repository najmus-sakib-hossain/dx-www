...
mod property_tests {
    ...
    fn test_dictionary_has_required_mappings() {
        ...
        for (abbrev, full) in required {
            assert_eq!(dict.expand(abbrev, ""), full,
                "Required mapping {} -> {} not found", abbrev, full);
            assert_eq!(dict.compress(full), abbrev,
                "Required reverse mapping {} -> {} not found", full, abbrev);
        }
    }
