        ...
        if seq_len == 2 {
            ...
            if code_point < 0x80 {
                return Err(Utf8ValidationError::new(
                    i,
                    Some(byte),
                    format!("Overlong encoding at position {}: 2-byte sequence for code point U+{:04X}", i, code_point),
                ));
            }
        } else if seq_len == 3 {
            ...
            if code_point < 0x800 {
                return Err(Utf8ValidationError::new(
                    i,
                    Some(byte),
                    format!("Overlong encoding at position {}: 3-byte sequence for code point U+{:04X}", i, code_point),
                ));
            }
            ...
            if code_point < 0x10000 {
                return Err(Utf8ValidationError::new(
                    i,
                    Some(byte),
                    format!("Overlong encoding at position {}: 4-byte sequence for code point U+{:04X}", i, code_point),
                ));
            }
            if code_point > 0x10FFFF {
                return Err(Utf8ValidationError::new(
                    i,
                    Some(byte),
                    format!("Code point U+{:04X} exceeds maximum U+10FFFF at position {}", code_point, i),
                ));
            }
        }
