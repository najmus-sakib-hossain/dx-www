...
mod llm_format {
    ...
    fn spec_boolean_values() {
        ...
        // Booleans serialize as + and -
        assert!(llm.contains("|+") || llm.contains(";+"));
        assert!(llm.contains("|-") || llm.contains(";-"));
    ...
    fn spec_array_values() {
        let mut doc = DxDocument::new();
        doc.context.insert("tags".into(), DxLlmValue::Arr(vec![
...
mod machine_format {
    ...
    fn spec_all_value_types() {
        ...
        doc.context.insert("arr".into(), DxLlmValue::Arr(vec![
            DxLlmValue::Num(1.0),
            DxLlmValue::Num(2.0),
        ]));
    ...
    fn spec_binary_header() {
        ...
        // Check magic bytes
        assert_eq!(&machine.data[0..4], b"DXMF");
    }
...
mod edge_cases {
    ...
    fn spec_nested_arrays() {
        let mut doc = DxDocument::new();
        doc.context.insert("nested".into(), DxLlmValue::Arr(vec![
