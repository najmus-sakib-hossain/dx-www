...
```bash
git clone https://github.com/dx-www/dx-serializer
cd dx-serializer/crates/dx-serializer
```
...
Follow our [architecture principles](#architecture-principles).
...
mod tests {
    ...
    fn test_feature() {
        // Test implementation
        assert_eq!(parse(b"test:value").unwrap(), expected);
    }
}
```
...
///
/// # Arguments
/// * `input` - DX format bytes (UTF-8)
///
/// # Returns
/// * `Ok(DxValue)` - Parsed data
/// * `Err(DxError)` - Parse error with position
///
/// # Examples
/// ```
...
1. **Automated Checks:** CI runs tests, lints, benchmarks
...
1. **Search:** Check if idea already exists
2. **Create Issue:** Use feature request template
3. **Describe:** What problem does it solve?
4. **Discuss:** Maintainers will discuss feasibility
...
1. **Search:** Check if bug already reported
2. **Create Issue:** Use bug report template
3. **Reproduce:** Minimal reproduction steps
4. **Context:** Rust version, OS, error message
...
```
Before:
  parse_simple     1.9 µs ± 0.05 µs

After:
  parse_simple     1.1 µs ± 0.03 µs
...
- **Clarifications:** Improve existing docs
- **Examples:** Add code examples
- **Guides:** Write tutorials
...
- **GitHub Issues:** [Ask questions](https://github.com/dx-www/dx-serializer/issues)
- **Discussions:** [Community forum](https://github.com/dx-www/dx-serializer/discussions)
...
**Thank you for contributing to DX Serializer!** 🦀⚡
