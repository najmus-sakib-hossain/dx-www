...
"#,
    ...
    // JSON → DX
    convert_and_display("JSON", test_config.json, |s| json_to_dx(s));

    // YAML → DX
    convert_and_display("YAML", test_config.yaml, |s| yaml_to_dx(s));

    // TOML → DX
    convert_and_display("TOML", test_config.toml, |s| toml_to_dx(s));
...
fn convert_and_display<F>(format: &str, input: &str, mut converter: F)
where
