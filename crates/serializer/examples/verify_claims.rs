...
fn verify_size_claims() {
    ...
    let json_bytes = sizes.iter().find(|(n, _, _)| n.contains("JSON")).map(|(_, b, _)| *b).unwrap_or(1);
    let toon_bytes = sizes.iter().find(|(n, _, _)| n.contains("TOON")).map(|(_, b, _)| *b).unwrap_or(1);
    let json_tokens = sizes.iter().find(|(n, _, _)| n.contains("JSON")).map(|(_, _, t)| *t).unwrap_or(1);
    let toon_tokens = sizes.iter().find(|(n, _, _)| n.contains("TOON")).map(|(_, _, t)| *t).unwrap_or(1);

    for (name, bytes, tokens) in &sizes {
        let vs_json = json_bytes as f64 / *bytes as f64;
        let vs_toon = toon_bytes as f64 / *bytes as f64;
        println!("  {:22} {:6}   {:12}   {:5.2}x    {:5.2}x", 
            name, bytes, tokens, vs_json, vs_toon);
    }

    // Verify claim
    let llm_bytes = sizes.iter().find(|(n, _, _)| n.contains("LLM")).map(|(_, b, _)| *b).unwrap_or(0);
