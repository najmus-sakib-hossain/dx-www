...
mod compile_benchmarks {
    ...
    pub fn asset_bundle_benchmark(c: &mut Criterion) {
        ...
        let medium_asset = EmbeddedAsset::new("medium.bin", &medium_data, "application/octet-stream").unwrap();
...
mod macro_benchmarks {
    ...
    pub fn macro_value_benchmark(c: &mut Criterion) {
        ...
        group.bench_function("from_json", |b| {
            b.iter(|| MacroValue::from_json(black_box(&json)))
        });

        group.bench_function("to_js_literal", |b| {
            b.iter(|| value.to_js_literal())
        });
    ...
    pub fn macro_context_benchmark(c: &mut Criterion) {
        ...
        group.bench_function("env_lookup", |b| {
            b.iter(|| ctx.env(black_box("TEST_VAR")))
        });
        ...
        group.bench_function("execute_cached", |b| {
            b.iter(|| {
                ctx.execute_cached("bench_key", |_| Ok(MacroValue::Integer(42)))
            })
        });
