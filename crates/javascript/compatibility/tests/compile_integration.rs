...
fn test_target_from_str_variants() {
    ...
    for (input, expected) in test_cases {
        assert_eq!(Target::from_str(input), expected, "Failed for input: {}", input);
    }
}
...
fn test_target_cross_compilation_detection() {
    ...
    let cross_compile_count = all_targets
        .iter()
        .filter(|t| t.requires_cross_compilation())
        .count();
...
fn test_asset_bundle_metadata_tracking() {
    ...
    assert_eq!(
        bundle.metadata.total_original_size,
...
fn test_asset_bundle_serialization_roundtrip() {
    let mut bundle = AssetBundle::new(Target::LinuxArm64, "index.ts", "serialization-test", "3.0.0");
    
    let asset = EmbeddedAsset::new("data.json", b"{\"key\": \"value\"}", "application/json").unwrap();
...
fn test_compiler_compile_with_temp_file() {
    ...
    assert!(output.bundle.assets.len() >= 1);
}
...
fn test_compiler_embed_files() {
    ...
    let options = CompileOptions::new(&entry_path)
        .embed_files(&[&asset1_path, &asset2_path]);
...
fn test_runtime_from_bundle() {
    ...
    let entry_asset = EmbeddedAsset::new("index.js", b"console.log('Hello');", "application/javascript").unwrap();
    let data_asset = EmbeddedAsset::new("data.json", b"{\"key\": \"value\"}", "application/json").unwrap();
