...
impl MacroContext {
    ...
    pub fn read_file(&self, path: impl AsRef<Path>) -> MacroResult<String> {
        if !self.config.allow_fs {
            return Err(MacroError::FileAccess(
                "File system access is disabled".to_string(),
            ));
        }
    ...
    pub fn env(&self, key: &str) -> MacroResult<Option<String>> {
        if !self.config.allow_env {
            return Err(MacroError::EnvVar(
                "Environment variable access is disabled".to_string(),
            ));
        }
    ...
    fn resolve_path(&self, path: &Path) -> MacroResult<PathBuf> {
        ...
        if !self.config.allowed_paths.is_empty() {
            let canonical = resolved
                .canonicalize()
                .map_err(|e| MacroError::FileAccess(e.to_string()))?;
...
impl MacroDefinition {
    ...
    pub fn new(name: impl Into<String>, module: impl Into<String>, function: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            module: module.into(),
...
mod tests {
    ...
    fn test_macro_registry() {
        ...
        registry.register(
            MacroDefinition::new("test", "test_module", "test_fn")
    ...
    fn test_macro_expansion() {
        ...
        let expansion = MacroExpansion::new(value.clone())
            .with_location("test.ts", 10, 5);
