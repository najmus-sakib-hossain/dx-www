...
pub async fn serve(options: SimpleServeOptions) -> BunResult<Server> {
    ...
    let addr: SocketAddr = format!("{}:{}", hostname, options.port)
        .parse()
        .map_err(|e| BunError::Serve(format!("Invalid address: {}", e)))?;

    let listener = TcpListener::bind(addr)
        .await
        .map_err(|e| BunError::Serve(format!("Failed to bind: {}", e)))?;
...
mod tests {
    ...
    async fn test_serve_creates_server() {
        let options = SimpleServeOptions::new(0, |_req| {
            Box::pin(async move { Response::ok("test") })
        });
