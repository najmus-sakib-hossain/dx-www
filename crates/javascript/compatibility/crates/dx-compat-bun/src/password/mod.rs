...
fn hash_bcrypt(password: &str, options: BcryptOptions) -> BunResult<String> {
    bcrypt::hash(password, options.cost).map_err(|e| BunError::Password(format!("bcrypt hash failed: {}", e)))
}
...
fn verify_bcrypt(password: &str, hash: &str) -> BunResult<bool> {
    bcrypt::verify(password, hash).map_err(|e| BunError::Password(format!("bcrypt verify failed: {}", e)))
}
