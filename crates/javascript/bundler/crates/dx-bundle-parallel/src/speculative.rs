...
impl SpeculativeBundler {
    ...
    pub fn bundle(&self, entries: &[PathBuf], options: &ParallelOptions) -> BundleResult<ParallelBundle> {
        ...
        // Collect results
        let modules: Vec<TransformedModule> = self.transformed
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
        
        let entry_ids: Vec<ModuleId> = entries
            .iter()
            .map(|path| PathHasher::hash(path))
            .collect();
    ...
    fn bundle_parallel(&self, entries: &[PathBuf], options: &ParallelOptions) -> BundleResult<()> {
        ...
        let scan_results: Vec<_> = entries
            ...
            .map(|path| {
                let source = std::fs::read(path)
                    .map_err(|e| BundleError::module_not_found(path))?;
                let scan = scan_source(&source);
                Ok((path.clone(), source, scan))
            })
        ...
        for (path, source, scan) in scan_results {
            ...
            self.resolved.insert(module_id, ResolvedModule {
                id: module_id,
                path: path.clone(),
                source: source.clone(),
                imports: vec![],
            ...
            self.pending.insert(module_id, path.clone());
        ...
        let pending_list: Vec<_> = self.pending.iter()
            .map(|entry| (*entry.key(), entry.value().clone()))
            .collect();
        
        pending_list
            .par_iter()
            .try_for_each(|(module_id, path)| {
                self.process_module(*module_id, path, options)
            })?;
        
        Ok(())
    }
    
    fn process_module(&self, module_id: ModuleId, path: &Path, options: &ParallelOptions) -> BundleResult<()> {
        // Check cache first
        if let Some(ref cache) = *self.cache {
            if let Some(cached) = cache.get(path) {
                self.transformed.insert(module_id, TransformedModule {
                    id: module_id,
        ...
        // Read source
        let source = std::fs::read(path)
            .map_err(|_| BundleError::module_not_found(path))?;
        ...
        let import_ids: Vec<ModuleId> = scan.imports
            .iter()
            .filter_map(|&pos| {
        ...
        if let Some(ref cache) = *self.cache {
            ...
            cache.put(path, dx_bundle_cache::CachedTransform {
                content_hash: hash,
                transformed,
                imports: import_ids,
