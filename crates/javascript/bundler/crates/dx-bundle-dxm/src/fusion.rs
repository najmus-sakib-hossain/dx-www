...
pub fn fuse(inputs: Vec<FusionInput>, config: &FusionConfig) -> Result<FusionResult, String> {
    ...
    let resolved: Vec<(usize, &[u8])> = inputs.iter()
        .filter_map(|input| {
            match input {
    ...
    let total_size: usize = resolved.iter()
...
pub fn fuse_with_imports(
    ...
    let mut inputs: Vec<FusionInput> = dependencies.into_iter()
        .map(|(_, dxm)| FusionInput::Dxm(dxm))
        .collect();
...
pub fn quick_bundle(
    ...
    // Read user code
    let user_code = fs::read(user_code_path)
