...
use crate::format::{DxmModule, fnv1a_hash};
use std::path::Path;
use std::fs;
...
pub fn atomize(source: &str, config: &AtomizerConfig) -> AtomizeResult {
    ...
    for (i, export_name) in exports.iter().enumerate() {
...
pub fn atomize_package(package_path: &Path, config: &AtomizerConfig) -> Result<AtomizeResult, String> {
    ...
    let main = extract_main_from_package_json(&pkg_json)
...
pub fn write_dxm(module: &DxmModule, output_path: &Path) -> Result<(), String> {
    ...
    fs::write(output_path, bytes)
        .map_err(|e| format!("Failed to write DXM: {}", e))
}

/// Read DXM module from file
pub fn read_dxm(path: &Path) -> Result<DxmModule, String> {
    let bytes = fs::read(path)
        .map_err(|e| format!("Failed to read DXM: {}", e))?;
    DxmModule::from_bytes(&bytes)
