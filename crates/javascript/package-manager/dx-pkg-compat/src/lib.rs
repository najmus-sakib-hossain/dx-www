...
impl PackageJson {
    ...
    pub fn read(path: &Path) -> Result<Self> {
        ...
        let pkg: PackageJson = serde_json::from_str(&content)
            .map_err(|e| dx_pkg_core::Error::Parse(format!("Invalid package.json: {}", e)))?;
        Ok(pkg)
    }
...
pub fn parse_npm_version(version_str: &str) -> Result<Version> {
    ...
    if parts.len() < 3 {
        return Err(dx_pkg_core::Error::InvalidVersion(format!(
            "Invalid version: {}",
            version_str
        )));
    }
    ...
    let major = parts[0]
        .parse()
        .map_err(|_| dx_pkg_core::Error::InvalidVersion(version_str.to_string()))?;
    let minor = parts[1]
        .parse()
        .map_err(|_| dx_pkg_core::Error::InvalidVersion(version_str.to_string()))?;
    let patch = parts[2]
