...
use dx_generator::{
    BinaryTemplate, Parameters, Renderer, RenderMode,
    Compiler, Generator,
};
...
fn bench_micro_render(c: &mut Criterion) {
    ...
    let template = BinaryTemplate::builder("greeting")
        .build();
    ...
    group.bench_function("simple_template", |b| {
        b.iter(|| {
            renderer.render(black_box(&template), black_box(&params))
        })
    });
...
fn bench_macro_render(c: &mut Criterion) {
    ...
    let template = BinaryTemplate::builder("component")
        .build();
...
fn bench_full_generation(c: &mut Criterion) {
    ...
    gen.compile_template("test", "Hello, {{name}}!").unwrap();
    
    let params = Parameters::new().set("name", "World");
    
    group.bench_function("generate", |b| {
        b.iter(|| {
            gen.generate(black_box("test"), black_box(&params))
        })
    });
...
fn bench_params(c: &mut Criterion) {
    ...
    group.bench_function("encode", |b| {
        b.iter(|| params.encode())
    });
    
    let encoded = params.encode();
    
    group.bench_function("decode", |b| {
        b.iter(|| Parameters::decode(black_box(&encoded)))
    });
