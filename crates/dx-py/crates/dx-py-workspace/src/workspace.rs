...
impl WorkspaceMember {
                ...
                if let Some(deps) = dx_py.get("dependencies") {
                    if let Some(table) = deps.as_table() {
                        for (name, value) in table {
                            if let Some(path_dep) = Self::parse_path_dep_entry(name, value, member_path) {
                    ...
                    for dep in arr {
                        if let Some(dep_str) = dep.as_str() {
                            ...
                            if dep_str.contains(" @ file://") {
                                if let Some(path_dep) = Self::parse_pep508_path_dep(dep_str, member_path) {
                                    path_deps.push(path_dep);
                                }
                            }
                        }
    ...
    fn parse_path_dep_entry(name: &str, value: &toml::Value, member_path: &Path) -> Option<PathDependency> {
        if let Some(table) = value.as_table() {
            if let Some(path_value) = table.get("path") {
                if let Some(path_str) = path_value.as_str() {
                    let path = PathBuf::from(path_str);
                    let editable = table.get("editable")
...
impl WorkspaceManager {
        ...
        for path in member_paths {
            match WorkspaceMember::load_with_root(&path, &self.root) {
                ...
                Err(e) => {
                    ...
                    eprintln!("Warning: Failed to load workspace member at {}: {}", path.display(), e);
                }
            }
        }
    ...
    pub fn resolve_path_dependency(&mut self, dep: &PathDependency) -> Result<Option<&WorkspaceMember>> {
    ...
    pub fn sorted_members(&mut self) -> Result<Vec<&WorkspaceMember>> {
        ...
        for member in members {
            in_degree.entry(&member.name).or_insert(0);
            graph.entry(&member.name).or_default();
        }
        ...
        for (member_name, deps) in &inter_deps {
            for dep in deps {
                ...
                graph.entry(member_name.as_str()).or_default().push(dep.as_str());
            }
        }
