...
impl WheelTag {
        ...
        let (name, version, build) = if parts.len() >= 6 {
            ...
            if potential_build.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(false) {
                (
                    parts[0].to_string(),
                    parts[1].to_string(),
                    Some(potential_build.to_string()),
                )
            } else {
    ...
    fn python_compatible(&self, env: &PlatformEnvironment) -> bool {
        ...
        self.python_tags
            .iter()
            .any(|t| env_tags.iter().any(|e| tags_match(t, e)))
    }
    ...
    pub fn specificity_score(&self, env: &PlatformEnvironment) -> u32 {
        ...
        if self.platform_tags.iter().any(|t| {
            env.platform_tags()
                .first()
                .map(|e| tags_match(t, e))
                .unwrap_or(false)
        }) {
            score += 500;
        }
    ...
    pub fn to_filename(&self) -> String {
        ...
        if let Some(ref build) = self.build {
            format!(
                "{}-{}-{}-{}-{}-{}.whl",
                self.name, self.version, build, python, abi, platform
            )
        } else {
            format!(
                "{}-{}-{}-{}-{}.whl",
                self.name, self.version, python, abi, platform
            )
        }
    }
}
...
mod tests {
    ...
    fn test_parse_cpython_wheel() {
        let wheel =
            WheelTag::parse("numpy-1.26.0-cp312-cp312-manylinux_2_17_x86_64.whl").unwrap();
