...
impl DplLockFile {
    ...
    pub fn lookup(&self, package_name: &str) -> Option<DplEntry> {
        ...
        for _ in 0..self.header.package_count {
            ...
            let entry: DplEntry = *bytemuck::from_bytes(&self.mmap[entry_offset..entry_offset + entry_size]);
...
impl<'a> Iterator for DplIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let entry: DplEntry = *bytemuck::from_bytes(
            &self.lock_file.mmap[entry_offset..entry_offset + entry_size],
        );
