...
async fn test_resolve_requests_package() {
    ...
    let deps = vec![
        DependencySpec::parse("requests>=2.28").unwrap(),
    ];
...
async fn test_resolve_with_markers() {
    ...
    let deps = vec![
        DependencySpec::parse("colorama; sys_platform == 'win32'").unwrap(),
    ];
...
async fn test_download_and_install_wheel() {
    ...
    assert!(six_py.exists() || site_packages_dir.path().join("six/__init__.py").exists(),
...
fn test_venv_creation() {
    use dx_py_project_manager::{VenvManager, RealVenvManager, PythonManager};
    use tempfile::TempDir;
    ...
    let venv_manager = RealVenvManager::new();
...
fn test_activation_scripts_generated() {
    use dx_py_project_manager::{VenvManager, Venv};
    use tempfile::TempDir;
    ...
    venv_manager.write_activation_scripts(&venv_path).unwrap();

    // Verify activation scripts exist
    #[cfg(unix)]
    {
        assert!(venv_path.join("bin/activate").exists(), "bash activate should exist");
        assert!(venv_path.join("bin/activate.fish").exists(), "fish activate should exist");
    }

    #[cfg(windows)]
    {
        assert!(venv_path.join("Scripts/activate.bat").exists(), "batch activate should exist");
        assert!(venv_path.join("Scripts/Activate.ps1").exists(), "PowerShell activate should exist");
    }
}
