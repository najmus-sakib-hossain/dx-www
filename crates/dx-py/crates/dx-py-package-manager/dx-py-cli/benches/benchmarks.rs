...
fn bench_version_comparison(c: &mut Criterion) {
    ...
    let candidates: Vec<PackedVersion> = (0..1000)
        .map(|i| PackedVersion::new(i / 100, (i / 10) % 10, i % 10))
        .collect();
...
fn bench_lockfile(c: &mut Criterion) {
    ...
    for num_packages in [10, 100, 500, 1000].iter() {
        ...
        group.throughput(Throughput::Elements(*num_packages as u64));
        group.bench_with_input(
            BenchmarkId::new("read", num_packages),
            &lock_path,
            |b, path| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("lookup", num_packages),
            num_packages,
            |b, n| {
                b.iter(|| {
                    for i in 0..*n {
...
fn bench_hint_cache(c: &mut Criterion) {
    ...
    let resolved: Vec<_> = (0..100)
        .map(|i| dx_py_package_manager::resolver::ResolvedPackage::new(
            &format!("pkg-{}", i),
