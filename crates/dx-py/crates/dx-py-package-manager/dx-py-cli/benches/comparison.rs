...
impl SystemInfo {
            ...
            "windows" => {
                    ...
                    .output()
                {
                    ...
                    if version.contains("10") || version.contains("11") {
                        return format!("Windows {}", if version.contains("11") { "11" } else { "10" });
                    }
                }
                "Windows".to_string()
            }
    ...
    fn detect_uv_version() -> Option<String> {
        ...
        if let Ok(output) = Command::new(uv_cmd)
            .args(["--version"])
            .output()
        {
...
impl BenchmarkResults {
    ...
    pub fn to_markdown_table(&self) -> String {
        ...
        for ((operation, scenario), (dx_py, uv)) in grouped {
            ...
            let cold_speedup = if dx_cold > 0.0 { uv_cold / dx_cold } else { 0.0 };
            let warm_speedup = if dx_warm > 0.0 { uv_warm / dx_warm } else { 0.0 };

            output.push_str(&format!(
                "| {} | {} | {:.0}ms | {:.0}ms | {:.1}x | {:.0}ms | {:.0}ms | {:.1}x |\n",
                operation, scenario,
                dx_cold, uv_cold, cold_speedup,
                dx_warm, uv_warm, warm_speedup
            ));
        }
