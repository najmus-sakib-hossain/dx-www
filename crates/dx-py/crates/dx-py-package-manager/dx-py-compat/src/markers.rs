...
impl<'a> Parser<'a> {
    ...
    fn parse_operator(&mut self) -> Result<CompareOp, MarkerParseError> {
        ...
        Err(MarkerParseError::InvalidOperator(
            remaining.chars().take(5).collect(),
        ))
    }
...
mod tests {
    ...
    fn test_parse_and_expression() {
        let expr = MarkerEvaluator::parse("python_version >= '3.8' and sys_platform == 'linux'").unwrap();
        match expr {
            MarkerExpr::And(_, _) => {}
    ...
    fn test_parse_or_expression() {
        let expr = MarkerEvaluator::parse("sys_platform == 'win32' or sys_platform == 'linux'").unwrap();
        match expr {
            MarkerExpr::Or(_, _) => {}
            _ => panic!("Expected Or"),
        }
