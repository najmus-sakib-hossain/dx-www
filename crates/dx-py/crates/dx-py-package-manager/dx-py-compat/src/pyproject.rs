...
pub enum LicenseField {
    ...
    Table { file: Option<String>, text: Option<String> },
}
...
pub fn convert_from_binary(binary: &[u8]) -> Result<PyProjectToml> {
    ...
    let project = if project_size > 0 {
        ...
        Some(serde_json::from_slice(project_bytes)
    ...
    let tool = if tool_size > 0 {
        ...
        Some(serde_json::from_slice(tool_bytes)
    ...
    let build_system = if build_system_size > 0 {
        let build_system_bytes = &binary[build_system_offset..build_system_offset + build_system_size];
        Some(serde_json::from_slice(build_system_bytes)
