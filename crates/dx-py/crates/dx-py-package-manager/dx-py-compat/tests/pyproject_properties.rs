...
fn arb_pyproject() -> impl Strategy<Value = PyProjectToml> {
    (
        prop::option::of(arb_project_section()),
        prop::option::of(arb_build_system()),
    )
        .prop_map(|(project, build_system)| PyProjectToml {
            project,
...
"#;
    ...
    assert_eq!(
        original.optional_dependencies("dev"),
        restored.optional_dependencies("dev")
    );
