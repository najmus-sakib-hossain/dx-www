...
impl IoUringReactor {
    ...
    fn build_sqe(&self, op: &IoOperation, user_data: u64) -> Result<io_uring::squeue::Entry> {
        let sqe = match op {
            ...
            IoOperation::Pread { fd, buf, offset, .. } => {
                opcode::Read::new(types::Fd(*fd), buf.as_ptr() as *mut u8, buf.len() as u32)
                    .offset(*offset)
                    .build()
                    .user_data(user_data)
            ...
            IoOperation::Pwrite { fd, buf, offset, .. } => {
                opcode::Write::new(types::Fd(*fd), buf.as_ptr(), buf.len() as u32)
                    .offset(*offset)
                    .build()
                    .user_data(user_data)
            ...
            IoOperation::Connect { fd, addr, .. } => {
                ...
                opcode::Connect::new(types::Fd(*fd), sockaddr, len)
                    .build()
                    .user_data(user_data)
            }
            ...
            IoOperation::Timeout { duration_ns, .. } => {
                ...
                opcode::Timeout::new(&ts as *const _)
                    .build()
                    .user_data(user_data)
            }

            IoOperation::Cancel { target_user_data, .. } => {
