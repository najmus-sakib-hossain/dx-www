...
use dx_py_gc::{LockFreeRefCount, EpochGc, CycleDetector};
use std::sync::Arc;
...
fn bench_cycle_detector_creation(c: &mut Criterion) {
    c.bench_function("cycle_detector_creation", |b| {
        b.iter(|| {
            black_box(CycleDetector::new())
        })
    });
}
    ...
    for num_workers in [1, 2, 4].iter() {
        group.bench_with_input(
            ...
            |b, &num_workers| {
                ...
                b.iter(|| {
                    black_box(detector.detect_cycles(num_workers))
                })
            },
        );
    }
