...
use crate::format::{DpmHeader, DpmError, ExportEntry, ImportEntry};
...
impl DpmLoader {
    ...
    pub fn load<P: AsRef<Path>>(&self, path: P) -> Result<Arc<LoadedModule>, DpmError> {
        ...
        let header: DpmHeader = unsafe {
            std::ptr::read_unaligned(mmap.as_ptr() as *const DpmHeader)
        };
        header.validate()?;
    ...
    fn load_export_table(mmap: &Mmap, header: &DpmHeader) -> Result<ExportTable, DpmError> {
        ...
        for i in 0..header.exports_count as usize {
            ...
            let entry: ExportEntry = unsafe {
                std::ptr::read_unaligned(mmap[start..].as_ptr() as *const ExportEntry)
            };
