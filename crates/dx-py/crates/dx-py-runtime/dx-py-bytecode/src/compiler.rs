...
impl DpbCompiler {
    ...
    fn build_dpb(&self, code: &CodeObject) -> Result<Vec<u8>, DpbError> {
        ...
        if code.flags.contains(CodeFlags::COROUTINE) || code.flags.contains(CodeFlags::ASYNC_GENERATOR) {
        ...
        output.extend_from_slice(unsafe {
            std::slice::from_raw_parts(
                &header as *const DpbHeader as *const u8,
                header_size,
            )
        });
        ...
        output[header_size - 32..header_size].copy_from_slice(hash.as_bytes());
    ...
    fn serialize_constant(&self, data: &mut Vec<u8>, constant: &Constant) {
        match constant {
            Constant::None => {
            ...
            Constant::Code(_) => {
                ...
                // TODO: Serialize nested code object
            }
            Constant::Ellipsis => {
