...
impl DpbModule {
    ...
    fn skip_constant(&self, data: &[u8], offset: usize) -> Option<usize> {
        if offset >= data.len() {
            return None;
        }
        ...
        match const_type {
            ...
            ConstantType::String | ConstantType::Bytes => {
                let len = u32::from_le_bytes(data[offset + 1..offset + 5].try_into().ok()?) as usize;
                Some(offset + 5 + len)
            }
            ConstantType::Tuple | ConstantType::FrozenSet => {
                let count = u32::from_le_bytes(data[offset + 1..offset + 5].try_into().ok()?) as usize;
            ...
            ConstantType::Code => {
                // TODO: Handle nested code objects
                Some(offset + 1)
            }
        }
    }
    ...
    fn parse_constant(&self, data: &[u8], offset: usize) -> Option<(Constant, usize)> {
        if offset >= data.len() {
            return None;
        }
