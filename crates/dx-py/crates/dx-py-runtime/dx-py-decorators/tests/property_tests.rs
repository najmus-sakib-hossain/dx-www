...
use dx_py_decorators::{
    InlineableDecorator, DecoratorInliner, InlineLruCache, DataclassInfo,
    inlineable::FunctionFlags,
    lru_cache::CacheKey,
...
mod inlining_tests {
    ...
    proptest! {
        ...
        fn prop_inlineable_detection(
            ...
            let test_name = if is_builtin && !builtin_names.is_empty() {
...
mod unit_tests {
    ...
    fn test_inliner_custom_decorator() {
        ...
        inliner.register_custom("trace", |_| {
            dx_py_decorators::inliner::InlineResult {
        ...
        let result = inliner.inline(
            &InlineableDecorator::Custom("trace".to_string()),
            &[],
            None,
        );
