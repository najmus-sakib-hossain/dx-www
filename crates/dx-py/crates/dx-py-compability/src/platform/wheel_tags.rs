...
use super::{Architecture, Os, Platform, Libc};
...
impl WheelTag {
    ...
    pub fn new(python: impl Into<String>, abi: impl Into<String>, platform: impl Into<String>, priority: u32) -> Self {
        Self {
            python: python.into(),
            abi: abi.into(),
...
impl WheelTagGenerator {
    ...
    fn generate_platform_tags(&self) -> Vec<String> {
        ...
        tags = tags.iter().map(|t| {
            if (t.starts_with("manylinux") || t.starts_with("musllinux")) && !t.ends_with(&arch_suffix) {
        ...
        tags.iter()
            .filter_map(|tag| {
                compatible_tags.iter()
                    .find(|t| t.python == tag.python && t.abi == tag.abi && t.platform == tag.platform)
