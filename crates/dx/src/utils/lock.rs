...
impl FileLock {
    ...
    fn try_acquire_internal(lock_path: &Path, lock_type: LockType) -> Result<Option<Self>, DxError> {
...
mod tests {
    ...
    fn test_lock_file_path() {
        ...
        assert_eq!(lock_path, PathBuf::from("/tmp/.test.txt.lock"));
    }
    ...
    fn test_exclusive_lock_blocks_second_exclusive() {
        ...
        let lock2 = FileLock::try_acquire(&test_file, LockType::Exclusive)
            .expect("Should not error");
        ...
        let lock3 = FileLock::try_acquire(&test_file, LockType::Exclusive)
            .expect("Should not error");
