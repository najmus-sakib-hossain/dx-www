...
impl CrashReporter {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            ...
            match Self::save_report(&report) {
                ...
                Err(e) => {
                    ...
                    eprintln!("║                                                              ║");
                    eprintln!("║ Error: {}                                                    ", report.panic_message);
                    eprintln!("╚══════════════════════════════════════════════════════════════╝");
                }
            }
        }));
    ...
    fn generate_report(panic_info: &PanicHookInfo) -> CrashReport {
        ...
        CrashReport::new(panic_message, panic_location)
            .with_system_info(SystemInfo::default())
    }
    ...
    fn display_crash_message(report_path: &PathBuf, report: &CrashReport) {
        eprintln!();
        eprintln!("╔══════════════════════════════════════════════════════════════╗");
        eprintln!("║                      DX CLI CRASHED                          ║");
        eprintln!("╠══════════════════════════════════════════════════════════════╣");
