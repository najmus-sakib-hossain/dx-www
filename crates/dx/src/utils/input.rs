...
impl ValidationError {
    ...
    pub fn new(field: impl Into<String>, value: impl Into<String>, expected: impl Into<String>) -> Self {
        Self {
            field: field.into(),
            value: value.into(),
    ...
    pub fn validate_port_str(port_str: &str) -> Result<u16, ValidationError> {
        let port: u16 = port_str.parse().map_err(|_| {
            ValidationError::new(
                "port",
                port_str,
                "a valid number between 1 and 65535",
    ...
    pub fn check_path_traversal(
        ...
        if path_str.contains("..") {
                ...
                Err(_) => {
                    ...
                    if Self::path_escapes_root(&resolved, &canonical_root) {
                        return Err(SecurityWarning {
                            ...
                            description: "Path traversal detected: path escapes project directory".to_string(),
            ...
            if !canonical_path.starts_with(&canonical_root) {
                return Err(SecurityWarning {
                    issue_type: SecurityIssueType::PathTraversal,
                    description: "Path traversal detected: resolved path is outside project directory".to_string(),
                    path: Some(path.to_path_buf()),
                });
            }
        }
