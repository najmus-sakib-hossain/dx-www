...
mod tests {
    ...
    fn test_patcher_creation() {
        let patcher = DeltaPatcher::new("/usr/bin/dx");
        assert_eq!(patcher.current_binary(), "/usr/bin/dx");
    }
    ...
    fn test_patch_result_success() {
        let result = PatchResult::Success {
            output_path: "/tmp/dx.new".to_string(),
            hash: "abc123".to_string(),
        };

        match result {
            PatchResult::Success { output_path, hash } => {
                assert_eq!(output_path, "/tmp/dx.new");
    ...
    fn test_replace_binary_not_found() {
        let patcher = DeltaPatcher::new("/nonexistent/dx");
        let result = patcher.replace_binary(Path::new("/nonexistent/new"));
