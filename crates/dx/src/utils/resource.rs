...
mod tests {
    ...
    fn test_register_temp_file() {
        ...
        rm.register_temp_file(PathBuf::from("/tmp/test1.tmp"));
        assert_eq!(rm.temp_file_count(), 1);
        
        rm.register_temp_file(PathBuf::from("/tmp/test2.tmp"));
    ...
    fn test_unregister_temp_file() {
        let rm = ResourceManager::new(4);
        let path = PathBuf::from("/tmp/test_unregister.tmp");
    ...
    proptest! {
        ...
        fn prop_temp_file_tracking(num_files in 0usize..20) {
            ...
            for i in 0..num_files {
                rm.register_temp_file(PathBuf::from(format!("/tmp/test_{}.tmp", i)));
            }
        ...
        fn prop_cleanup_clears_all_tracked_files(num_files in 0usize..10) {
            ...
            // Register files (not actual files, just paths)
            for i in 0..num_files {
                rm.register_temp_file(PathBuf::from(format!("/nonexistent/test_{}.tmp", i)));
            }
        ...
        fn prop_unregister_removes_specific_file(
            ...
            let paths: Vec<PathBuf> = (0..num_files)
                .map(|i| PathBuf::from(format!("/tmp/test_{}.tmp", i)))
                .collect();
