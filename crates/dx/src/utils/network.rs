...
use crate::utils::error::{with_retry, DxError, EnhancedError};
...
impl NetworkClient {
    ...
    pub async fn download_resumable(
        ...
        with_retry("download", max_retries, || {
            let dest = dest.clone();
            async move {
                Self::perform_download(url, &dest).await
            }
        })
        .await
    }
    ...
    pub fn is_offline() -> bool {
        ...
        if env::var("DX_OFFLINE").map(|v| v == "1" || v.to_lowercase() == "true").unwrap_or(false) {
            return true;
        }
...
mod tests {
    ...
    fn test_proxy_config_construction() {
        ...
        let config = ProxyConfig {
            ...
            no_proxy: vec!["localhost".to_string(), "127.0.0.1".to_string(), ".internal.com".to_string()],
        };
    ...
    fn test_extract_host() {
        assert_eq!(NetworkClient::extract_host("https://example.com/path"), Some("example.com".to_string()));
        assert_eq!(NetworkClient::extract_host("http://api.example.com:8080/v1"), Some("api.example.com".to_string()));
