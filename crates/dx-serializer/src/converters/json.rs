...
fn convert_object(obj: &serde_json::Map<String, Value>, prefix: &str, output: &mut String) -> Result<(), String> {
    ...
    if !simple_props.is_empty() {
        let optimized_props: Vec<(String, String)> = simple_props.iter()
        ...
        if let Some(Value::Object(first)) = arr.first() {
            ...
            for item in arr.iter() {
                if let Value::Object(row) = item {
                    let values: Vec<String> = first.keys()
