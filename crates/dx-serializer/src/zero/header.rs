...
/// DX-Zero file header (4 bytes)
#[repr(C, packed)]
#[derive(Clone, Copy)]
pub struct DxZeroHeader {
...
impl DxZeroHeader {
    ...
    pub const fn new() -> Self {
        Self {
            magic: crate::zero::MAGIC,
            version: crate::zero::VERSION,
    ...
    pub fn validate(&self) -> Result<(), HeaderError> {
        // Check magic bytes
        if self.magic != crate::zero::MAGIC {
            return Err(HeaderError::InvalidMagic {
                expected: crate::zero::MAGIC,
                found: self.magic,
            });
        }

        // Check version
        if self.version != crate::zero::VERSION {
            return Err(HeaderError::UnsupportedVersion {
                supported: crate::zero::VERSION,
                found: self.version,
            });
        }
...
impl Default for DxZeroHeader {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Debug for DxZeroHeader {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DxZeroHeader")
...
mod tests {
    ...
    fn test_header_validation() {
        ...
        assert!(DxZeroHeader::from_bytes(&bytes).is_ok());
    ...
    #[test]
    fn test_header_size() {
        assert_eq!(DxZeroHeader::size(), 4);
        assert_eq!(std::mem::size_of::<DxZeroHeader>(), 4);
    }
}
