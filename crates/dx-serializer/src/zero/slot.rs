...
impl Default for DxZeroSlot {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Debug for DxZeroSlot {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.is_inline() {
            let len = self.inline_len();
            let data = self.inline_data();
            if let Ok(s) = std::str::from_utf8(data) {
                f.debug_struct("DxZeroSlot")
                    .field("type", &"inline")
                    .field("len", &len)
                    .field("data", &s)
                    .finish()
...
mod tests {
    ...
    #[test]
    fn test_slot_eq() {
        let slot = DxZeroSlot::inline_from_bytes(b"test").unwrap();
    ...
    #[test]
    fn test_slot_size() {
        assert_eq!(std::mem::size_of::<DxZeroSlot>(), 16);
    }
}
