...
/// 
/// Demonstrates that DX-Hyper is THE UNIVERSAL FORMAT for:
...
fn main() {
    println!("\n╔════════════════════════════════════════════════════════╗");
    println!("║  DX-Hyper: THE UNIVERSAL FORMAT                       ║");
        ...
        Ok(json_content) => {
            ...
            match json_to_dx(&json_content) {
                Ok(dx_hyper) => {
                    ...
                    println!("\n✨ DX-Hyper Result:");
                    println!("   Size: {} bytes (~{}% of JSON)", dx_bytes, (dx_bytes * 100 / json_bytes));
                    println!("   Tokens: ~{} ({:.1}× better!)", dx_tokens, token_ratio);
                    println!("   Byte efficiency: {:.2}× smaller", byte_ratio);
                    
                    // Show first 300 chars
                    println!("\n📄 DX-Hyper Output (preview):");
                    println!("   {}", truncate(&dx_hyper, 300));
...
fn test_synthetic_data() {
    ...
    match json_to_dx(test_json) {
        Ok(dx_hyper) => {
            ...
            println!("DX-Hyper: {} bytes, ~{} tokens", dx_bytes, dx_tokens);
...
fn demonstrate_universal_format() {
    println!("\n\n═══ TEST 2: Why DX-Hyper is UNIVERSAL ═══\n");
    
    println!("❌ Binary Formats (DX-Apex, Protocol Buffers, etc.):");
    ...
    println!("✅ DX-Hyper (Text Format):");
    println!("   Solution: Text-based, works for EVERYONE!");
    println!("   Example: name:alice#age:30#active:*");
    ...
    println!("   DX-Hyper        19%     15×     ✅ Yes  ← BEST!");
    println!("   Protocol Buf    15%     20×     ❌ NO!");
    println!("   DX-Apex (bin)   0.7%    1000×   ❌ NO!\n");
    
    println!("💡 Conclusion:");
    println!("   Binary is great for machines, terrible for LLMs.");
    println!("   DX-Hyper is the sweet spot for EVERYONE.\n");
}
