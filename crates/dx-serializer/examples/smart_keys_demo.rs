...
fn main() {
    ...
    for (full, expected_short) in popular_keys {
        ...
        let status = if compressed == expected_short { "✅" } else { "❌" };
        println!("  {} {:20} → {:10} {}", 
            status, full, compressed,
            if compressed == full { "(kept as-is)" } else { "(abbreviated)" }
        );
    }
    ...
    for (short, expected_full) in short_keys {
        ...
        let status = if expanded == expected_full { "✅" } else { "❌" };
        println!("  {} {:10} → {:20} {}", 
            status, short, expanded,
            if expanded == short { "(kept as-is)" } else { "(expanded)" }
        );
    }
    ...
    for key in &keys_to_test {
        ...
        let badge = if is_popular { "🔵 Popular" } else { "🟢 Custom" };
