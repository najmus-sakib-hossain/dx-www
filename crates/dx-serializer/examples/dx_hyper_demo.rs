...
hikes[3]{id,name,distanceKm,elevationGain,who,sunny}:
  ...
  3,Wildflower Loop,5.1,180,sam,true"#;
    ...
    println!("🔵 TOON Format ({} bytes, ~{} tokens):", 
        toon_format.len(), estimate_tokens(toon_format));
    println!("{}\n", toon_format);
    
    println!("🟢 DX-Hyper Simple ({} bytes, ~{} tokens):",
        hyper_simple.len(), estimate_tokens(&hyper_simple));
    println!("{}\n", hyper_simple);
    
    println!("🟣 DX-Hyper Compressed ({} bytes, ~{} tokens):",
        hyper_compressed.len(), estimate_tokens(&hyper_compressed));
    println!("{}\n", hyper_compressed);
    
    let savings = ((toon_format.len() - hyper_compressed.len()) as f64 / toon_format.len() as f64) * 100.0;
    let token_savings = ((estimate_tokens(toon_format) - estimate_tokens(&hyper_compressed)) as f64 
        / estimate_tokens(toon_format) as f64) * 100.0;
    
    println!("💰 Savings:");
    println!("   • Byte reduction: {:.1}% ({} → {} bytes)", 
        savings, toon_format.len(), hyper_compressed.len());
    println!("   • Token reduction: {:.1}% ({} → {} tokens)",
        token_savings, estimate_tokens(toon_format), estimate_tokens(&hyper_compressed));
    println!("   • Efficiency: {:.1}× better than TOON", 
...
fn make_hikes_data() -> DxValue {
    ...
    root.insert("friends".to_string(), DxValue::Array(DxArray { values: friends, is_stream: false }));
    ...
    root.insert("hikes".to_string(), DxValue::Array(DxArray { values: hikes, is_stream: false }));
...
fn demo_employee_records() {
    ...
    println!("🔵 TOON Estimated: {} bytes, ~{} tokens\n",
...
fn make_employee_data(count: usize) -> DxValue {
    ...
    for i in 0..count {
        ...
        emp.insert("department".to_string(), DxValue::String(departments[i % departments.len()].to_string()));
    ...
    DxValue::Array(DxArray { values: employees, is_stream: false })
}
...
fn demo_complex_config() {
    ...
    match decode_hyper(&hyper_compressed) {
        Ok(decoded) => {
            ...
            println!("   Decoded structure matches original: {:?}", 
...
fn make_complex_config() -> DxValue {
    ...
    root.insert("features".to_string(), DxValue::Array(DxArray { values: features, is_stream: false }));
