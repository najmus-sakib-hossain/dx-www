...
use dx_serializer::zero::{
    DxZeroBuilder, DxZeroHeader, DxZeroSlot, detect_format, DxFormat,
    FLAG_HAS_HEAP, FLAG_LITTLE_ENDIAN, INLINE_MARKER, HEAP_MARKER,
};
...
fn test_format_detection() {
    let mut buffer = Vec::new();
    let mut builder = DxZeroBuilder::new(&mut buffer, 0, 0);
    builder.finish();
