...
fn test_full_conversion_pipeline() {
  ...
  "dependencies": {
    ...
    let savings_pct = ((package_json.len() - dx.len()) as f64 / package_json.len() as f64) * 100.0;
    assert!(savings_pct > 30.0, "Should be at least 30% smaller (got {:.1}%)", savings_pct);
    println!("✅ package.json: {} → {} bytes ({:.1}% smaller)", package_json.len(), dx.len(), savings_pct);
...
fn test_compression_guarantees() {
    ...
    for (input, format) in configs {
        ...
        assert!(savings_pct > 30.0, 
                "{} should save >30% (actual: {:.1}%)", format, savings_pct);
...
fn test_language_code_optimization() {
  ...
  "languages": {
    ...
    assert!(dx.contains("js/ts") || dx.contains("javascript/typescript"), 
            "Should handle js/ts");
