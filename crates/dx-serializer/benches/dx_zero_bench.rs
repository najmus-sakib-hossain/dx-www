...
impl UserDxZero {
    ...
    fn name(&self) -> &str {
        ...
        if slot.is_inline() {
            ...
            unsafe {
                let ptr = (self as *const Self as *const u8).add(Self::HEAP_OFFSET + offset as usize);
...
fn bench_size_comparison(c: &mut Criterion) {
    ...
    let mut builder = DxZeroBuilder::new(&mut dx_buffer, UserDxZero::FIXED_SIZE, UserDxZero::SLOT_COUNT);
    builder.write_u64(0, user.id);
    builder.write_u32(8, user.age);
    builder.write_bool(12, user.active);
    builder.write_f64(13, user.score);
    ...
    println!("JSON:     {} bytes ({:.1}× larger)", json_size, json_size as f64 / dx_size as f64);
