...
impl RuntimeDetector {
    ...
    fn detect_in_path(&self, path: &PathBuf) -> Result<Vec<PythonRuntime>, DetectionError> {
        let mut runtimes = Vec::new();
        ...
        let python_names = if cfg!(windows) {
            ...
            vec!["python", "python3", "python3.8", "python3.9", "python3.10", "python3.11", "python3.12", "python3.13"]
        };
            ...
            if let Ok(output) = Command::new(cmd).arg("--version").output() {
                if output.status.success() {
                    if let Ok(runtime) = self.detect_runtime_from_command(cmd) {
                        if runtime.version.is_supported() && !runtimes.iter().any(|r: &PythonRuntime| r.executable == runtime.executable) {
                            runtimes.push(runtime);
                        }
                    }
                }
    ...
    fn get_version(&self, exe_path: &PathBuf) -> Result<PythonVersion, DetectionError> {
        ...
        PythonVersion::parse(version_part)
            .map_err(|e| DetectionError::ParseError(e.to_string()))
    }

    /// Determine installation source from path
    fn determine_source(&self, exe_path: &PathBuf) -> InstallationSource {
        let path_str = exe_path.to_string_lossy().to_lowercase();

        if path_str.contains("pyenv") {
            InstallationSource::Pyenv
        } else if path_str.contains("conda") || path_str.contains("miniconda") || path_str.contains("anaconda") {
    ...
    fn get_architecture(&self, exe_path: &PathBuf) -> Result<Architecture, DetectionError> {
        ...
        Ok(Architecture::from_str(arch_str.trim()))
    }
...
"#;
    ...
    pub fn find_version(&self, major: u8, minor: u8) -> Result<Option<PythonRuntime>, DetectionError> {
        let runtimes = self.detect_all()?;
        Ok(runtimes.into_iter().find(|r| r.version.major == major && r.version.minor == minor))
    }
