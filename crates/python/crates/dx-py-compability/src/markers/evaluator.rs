...
use super::{MarkerError, MarkerExpr, MarkerOp, MarkerValue, MarkerParser, MarkerCache};
...
impl MarkerEnvironment {
    ...
    pub fn get(&self, name: &str) -> Option<&str> {
        match name {
            ...
            "platform_python_implementation" | "platform.python_implementation" => Some(&self.platform_python_implementation),
...
impl MarkerEvaluator {
    ...
    fn compare(&self, left: &str, op: &MarkerOp, right: &str) -> Result<bool, MarkerError> {
        match op {
            ...
            MarkerOp::LessEqual => Ok(self.version_compare(left, right) != std::cmp::Ordering::Greater),
            MarkerOp::GreaterThan => Ok(self.version_compare(left, right) == std::cmp::Ordering::Greater),
            MarkerOp::GreaterEqual => Ok(self.version_compare(left, right) != std::cmp::Ordering::Less),
...
mod tests {
    ...
    fn test_evaluate_and() {
        ...
        assert!(evaluator.evaluate("python_version >= '3.8' and sys_platform == 'linux'").unwrap());
        assert!(!evaluator.evaluate("python_version >= '3.8' and sys_platform == 'win32'").unwrap());
    }
    ...
    fn test_evaluate_or() {
        ...
        assert!(evaluator.evaluate("sys_platform == 'linux' or sys_platform == 'darwin'").unwrap());
        assert!(!evaluator.evaluate("sys_platform == 'win32' or sys_platform == 'darwin'").unwrap());
    }
