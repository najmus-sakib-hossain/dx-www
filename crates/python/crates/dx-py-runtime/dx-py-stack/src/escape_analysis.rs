...
impl EscapeAnalyzer {
    ...
    pub fn analyze(&mut self, bytecode: &[u8]) {
        ...
        for (&offset, site) in &self.alloc_sites {
            ...
            self.escape_info.insert(offset, EscapeInfo {
                escapes,
                reason: if escapes { Some(EscapeReason::Returned) } else { None },
    ...
    fn mark_escaping(&mut self, bytecode: &[u8]) {
        ...
        while (offset as usize) < bytecode.len() {
            ...
            match opcode {
                ...
                0x70 | 0x71 | 0x72 | 0x76 => { // Call variants
    ...
    fn opcode_size(&self, opcode: u8) -> u32 {
        match opcode {
            // No argument
            0x15 | 0x12 | 0x13 | 0x16 | 0x56 | 0x57 | 0x59 | 0x5A |
            0x20..=0x30 | 0x40..=0x4A | 0xF0 => 1,
            // 1-byte argument
