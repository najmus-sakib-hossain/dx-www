...
use dx_py_stack::{
    EscapeAnalyzer, StackTuple, StackList, TaggedValue,
    escape_analysis::AllocKind,
};
...
mod escape_analysis_tests {
    ...
    proptest! {
        ...
        fn prop_finds_all_allocations(
            tuple_offsets in prop::collection::vec(0u32..100, 0..5),
            list_offsets in prop::collection::vec(0u32..100, 0..5)
        ) {
            ...
            // All allocation sites should be found
            for &offset in &tuple_offsets {
                if (offset as usize) < bytecode.len() - 1 {
