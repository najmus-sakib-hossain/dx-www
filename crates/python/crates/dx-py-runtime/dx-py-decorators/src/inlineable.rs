    ...
    pub fn parse(name: &str, args: &[(&str, &str)]) -> Option<Self> {
        match name {
            ...
            "dataclass" | "dataclasses.dataclass" => {
                ...
                Some(Self::Dataclass { frozen, slots, eq, order, hash })
            }
...
mod tests {
    ...
    fn test_parse_dataclass() {
        let dec = InlineableDecorator::parse(
            "dataclass",
    ...
    fn test_flags() {
        assert_eq!(
            InlineableDecorator::StaticMethod.get_flags(),
            FunctionFlags::STATIC_METHOD
        );
        assert_eq!(
            InlineableDecorator::Jit.get_flags(),
            FunctionFlags::IMMEDIATE_JIT
        );
    }
}
