...
use uuid::Uuid;
use crate::{EntangledObject, SharedMemoryRegion, EntangledError};
...
impl EntangledHandle {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        let size = u64::from_le_bytes(
            bytes[offset_start + 9..offset_start + 17].try_into().ok()?
        );
...
mod tests {
    ...
    fn test_handle_from_object() {
        let name = format!("test_handle_{}", std::process::id());
