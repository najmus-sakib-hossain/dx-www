...
impl SharedMemoryRegion {
    ...
    pub fn allocate(&self, size: usize, alignment: usize) -> Result<u64, EntangledError> {
        ...
        loop {
            ...
            if header.alloc_offset.compare_exchange(
                current,
                new_offset as u64,
                Ordering::AcqRel,
                Ordering::Acquire,
...
mod tests {
    ...
    fn test_create_region() {
        let name = format!("test_create_{}", std::process::id());
        ...
        // Cleanup
        std::fs::remove_file(region.path.clone()).ok();
    }
    
    #[test]
    fn test_allocate_and_write() {
        let name = format!("test_alloc_{}", std::process::id());
        let region = SharedMemoryRegion::create(&name, DEFAULT_SIZE).unwrap();
    ...
    fn test_open_existing() {
        let name = format!("test_open_{}", std::process::id());
    ...
    #[test]
    fn test_alignment() {
        let name = format!("test_align_{}", std::process::id());
        let region = SharedMemoryRegion::create(&name, DEFAULT_SIZE).unwrap();
