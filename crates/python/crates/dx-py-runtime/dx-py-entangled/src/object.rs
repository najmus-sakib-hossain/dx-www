...
use uuid::Uuid;
use crate::{SharedMemoryRegion, EntangledError};
...
impl EntangledObject {
    ...
    pub fn open(
        ...
        let (id, type_info, size) = unsafe {
            ...
            let type_info = TypeInfo::from_u8((*header_ptr).type_info)
                .ok_or(EntangledError::TypeMismatch)?;
    ...
    pub fn write(&self, data: &[u8], expected_version: u64) -> Result<u64, EntangledError> {
        ...
        if header.version.compare_exchange(
            expected_version,
            new_version,
            Ordering::AcqRel,
            Ordering::Acquire,
...
mod tests {
    ...
    fn create_test_region() -> Arc<SharedMemoryRegion> {
        let name = format!("test_obj_{}", std::process::id());
        Arc::new(SharedMemoryRegion::create(&name, 1024 * 1024).unwrap())
    }
    ...
    fn test_version_conflict() {
        ...
        obj.write(&vec![1u8; 50], 1).unwrap();
        
        // Second write with old version fails
        let result = obj.write(&vec![2u8; 50], 1);
    ...
    fn test_cas_write() {
        ...
        let success = obj.cas_write(&data, &vec![1u8; 20]).unwrap();
        assert!(success);
        
        // CAS with wrong expected value
        let success = obj.cas_write(&data, &vec![2u8; 20]).unwrap();
        assert!(!success);
    }
