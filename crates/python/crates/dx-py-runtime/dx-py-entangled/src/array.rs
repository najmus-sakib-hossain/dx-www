...
use std::sync::Arc;
use crate::{SharedMemoryRegion, EntangledObject, EntangledHandle, EntangledError};
...
mod tests {
    ...
    fn create_test_region() -> Arc<SharedMemoryRegion> {
        let name = format!("test_array_{}", std::process::id());
    ...
    fn test_add_scalar() {
        ...
        let arr = EntangledArray::from_f64(region, &data, &[3]).unwrap();
    ...
    fn test_handle_roundtrip() {
        ...
        let arr2 = EntangledArray::open(region, handle.offset).unwrap();
