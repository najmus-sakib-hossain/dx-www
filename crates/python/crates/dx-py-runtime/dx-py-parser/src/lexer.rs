...
impl<'a> Lexer<'a> {
    ...
    fn scan_number(&mut self) -> ParseResult<Token> {
        ...
        if is_float || has_exponent {
            let value: f64 = clean_text.parse().map_err(|_| {
            ...
            let value: i64 = clean_text.parse().map_err(|_| {
                ParseError::InvalidNumber {
                    location,
    ...
    fn scan_string(&mut self) -> ParseResult<Token> {
        ...
        let quote = self.advance().ok_or_else(|| {
            ParseError::unexpected_eof(location, "expected string quote")
        })?;
...
mod tests {
    ...
    fn test_numbers() {
        ...
        assert!(matches!(lexer.next_token().unwrap().kind, TokenKind::Float(f) if (f - 3.14).abs() < 0.001));
