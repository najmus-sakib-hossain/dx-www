...
impl SharedArrayHandle {
    ...
    pub fn as_slice<'a>(&self, arena: &'a SharedMemoryArena) -> Result<&'a [u8], SharedMemoryError> {
        arena.get(self.offset, self.metadata.byte_size())
    }
    
    /// Get the array data as a typed slice
    pub fn as_typed_slice<'a, T>(&self, arena: &'a SharedMemoryArena) -> Result<&'a [T], SharedMemoryError> {
