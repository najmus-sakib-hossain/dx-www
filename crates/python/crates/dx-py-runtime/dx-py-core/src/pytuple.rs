...
impl PyTuple {
    ...
    pub fn getitem(&self, index: i64) -> CoreResult<PyValue> {
        ...
        if idx < 0 || idx >= len {
            return Err(CoreError::IndexError(format!(
                "tuple index out of range: {}", index
            )));
        }
    ...
    pub fn index(&self, value: &PyValue) -> CoreResult<usize> {
        self.elements
            ...
            .ok_or_else(|| CoreError::ValueError("value not in tuple".into()))
    }
    ...
    pub fn hash(&self) -> CoreResult<u64> {
    ...
    /// Hash a single value
    fn hash_value(value: &PyValue) -> CoreResult<u64> {
        match value {
            PyValue::None => Ok(0),
