...
use crate::{
    FunctionSignature, CachedArtifact, CompilationTier,
    CodeStorage, PccError,
};
...
impl PersistentCompilationCache {
    ...
    pub fn get(&self, signature: &FunctionSignature) -> Option<(&CachedArtifact, *const u8)> {
        ...
        if let Some(mut entry) = self.index.get_mut(&key) {
                ...
                && entry.signature.type_profile_hash == signature.type_profile_hash
            {
                ...
                let artifact_ref = unsafe {
                    &*(&entry.artifact as *const CachedArtifact)
                };
    ...
    pub fn invalidate_source(&self, source_hash: &[u8; 32]) {
        self.index.retain(|_, entry| {
            entry.signature.source_hash != *source_hash
        });
    }
    ...
    pub fn cleanup(&self) -> Result<(), PccError> {
        ...
        let mut entries: Vec<(u128, u64, u32)> = self.index
            .iter()
    ...
    fn parse_index_entry(data: &[u8]) -> Option<(FunctionSignature, CachedArtifact)> {
        ...
        let artifact_len = u32::from_le_bytes(
            data[artifact_offset..artifact_offset + 4].try_into().ok()?
        ) as usize;
...
mod tests {
    ...
    fn test_persistence() {
        ...
        {
            let cache = PersistentCompilationCache::open(temp.path()).unwrap();
            cache.save(sig.clone(), CompilationTier::Optimized, &[0xCC; 100], vec![1, 2, 3]).unwrap();
            cache.flush().unwrap();
        }
    ...
    fn test_stats() {
        ...
        for i in 0..10 {
            ...
            let tier = if i % 2 == 0 { CompilationTier::Baseline } else { CompilationTier::Optimized };
            cache.save(sig, tier, &vec![0x90; 100], vec![]).unwrap();
        }
