...
use crate::format::{
    DpmHeader, DpmFlags, DpmError, ExportEntry, ExportKind, ImportEntry, ImportFlags,
    DPM_MAGIC, DPM_VERSION,
};
...
impl DpmCompiler {
    ...
    pub fn compile(&mut self, module: &ModuleDefinition) -> Result<Vec<u8>, DpmError> {
        ...
        let export_defs: Vec<_> = module.exports.iter()
            .enumerate()
        ...
        header.flags = if module.is_package { DpmFlags::IS_PACKAGE } else { DpmFlags::empty() };
        ...
        header.type_annotations_offset = if types_size > 0 { types_offset as u32 } else { 0 };
        ...
        output.extend_from_slice(&header.flags.bits().to_le_bytes());
        output.extend_from_slice(&header._reserved.to_le_bytes());
        output.extend_from_slice(&header.imports_offset.to_le_bytes());
        output.extend_from_slice(&header.exports_offset.to_le_bytes());
        output.extend_from_slice(&header.functions_offset.to_le_bytes());
        output.extend_from_slice(&header.classes_offset.to_le_bytes());
        output.extend_from_slice(&header.constants_offset.to_le_bytes());
        output.extend_from_slice(&header.type_annotations_offset.to_le_bytes());
        output.extend_from_slice(&header.init_bytecode_offset.to_le_bytes());
        output.extend_from_slice(&header.imports_count.to_le_bytes());
        output.extend_from_slice(&header.exports_count.to_le_bytes());
        output.extend_from_slice(&header.export_hash_seed.to_le_bytes());
        // Pad to 64 bytes before hash
        while output.len() < 64 - 32 {
            output.push(0);
        }
        // Hash placeholder
        ...
        for import in &module.imports {
            ...
            let symbol_offset = import.symbol_name.as_ref()
        ...
        for (i, export) in module.exports.iter().enumerate() {
            ...
            output.extend_from_slice(&name_hash.to_le_bytes());
            output.extend_from_slice(&name_offset.to_le_bytes());
            output.push(export.kind as u8);
            output.extend_from_slice(&[0u8; 3]); // reserved
            output.extend_from_slice(&(export_offsets[i] as u32).to_le_bytes());
        }
        ...
        output[64 - 32..64].copy_from_slice(content_hash.as_bytes());
    ...
    pub fn extract_imports(source: &str) -> Vec<ImportDef> {
        ...
        for line in source.lines() {
            ...
            if line.starts_with("import ") {
                let module = line[7..].trim().split_whitespace().next().unwrap_or("");
                if !module.is_empty() {
