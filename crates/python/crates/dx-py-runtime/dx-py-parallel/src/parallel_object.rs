...
impl ParallelPyObject {
    ...
    pub fn type_tag(&self) -> PyTypeTag {
        PyTypeTag::from_u8(self.type_tag.load(Ordering::Acquire))
            .unwrap_or(PyTypeTag::Custom)
    }
    ...
    pub fn cas_field<T: Copy + PartialEq>(
        field: &AtomicU64,
        expected: T,
        new: T,
    ) -> Result<T, T>
        ...
        <T as TryFrom<u64>>::Error: std::fmt::Debug,
    {
        ...
        match field.compare_exchange(expected_u64, new_u64, Ordering::AcqRel, Ordering::Acquire) {
            Ok(_) => Ok(expected),
            Err(actual) => Err(T::try_from(actual).expect("Invalid field value")),
        }
    }
}
