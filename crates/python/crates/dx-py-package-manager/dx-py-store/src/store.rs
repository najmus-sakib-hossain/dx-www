...
impl PackageStore {
    ...
    pub fn get(&self, hash: &[u8; 32]) -> StoreResult<Arc<MappedPackage>> {
        ...
        let mmap = unsafe {
            Mmap::map(&file).map_err(|e| StoreError::MmapFailed(e.to_string()))?
        };
    ...
    pub fn store_package(
        ...
        // Build header
        let header = PackageIndexHeader::new(
            index.file_count(),
            index_offset,
            data_offset,
            total_size,
    ...
    fn list_recursive(&self, dir: &Path, hashes: &mut Vec<[u8; 32]>) -> StoreResult<()> {
        if !dir.exists() {
            return Ok(());
        }
...
fn create_symlink(src: &Path, dst: &Path) -> StoreResult<()> {
    std::os::unix::fs::symlink(src, dst).map_err(|e| {
        StoreError::SymlinkFailed(format!(
            "{} -> {}: {}",
            src.display(),
            dst.display(),
            e
