    ...
    (
        ...
        .prop_map(
            |(
                ...
                python_version[..pv_bytes.len().min(15)].copy_from_slice(&pv_bytes[..pv_bytes.len().min(15)]);

                let mut platform = [0u8; 32];
                let pl_bytes = platform_str.as_bytes();
                platform[..pl_bytes.len().min(31)].copy_from_slice(&pl_bytes[..pl_bytes.len().min(31)]);
