...
fn bench_project_hash_computation(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("compute_hash", size),
            &packages,
            |b, pkgs| {
                b.iter(|| {
                    black_box(LayoutCache::compute_project_hash(pkgs))
                });
            },
        );
    }
...
fn bench_dpl_lookup(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 500, 1000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, name| {
                b.iter(|| {
                    black_box(lock_file.lookup(name))
                });
            },
        );
    }
...
fn bench_package_store_access(c: &mut Criterion) {
    ...
    group.bench_function("contains", |b| {
        b.iter(|| {
            black_box(store.contains(&hash))
        });
    });
    
    group.bench_function("get_path", |b| {
        b.iter(|| {
            black_box(store.get_path(&hash))
        });
    });
    
    group.bench_function("get_file", |b| {
        b.iter(|| {
            black_box(store.get_file(&hash, "test/__init__.py"))
        });
    });
        ...
        group.bench_with_input(
            ...
            |b, hash| {
                b.iter_with_setup(
                    ...
                    |target| {
                        black_box(cache.install_cached(hash, &target))
                    },
                );
            },
        );
