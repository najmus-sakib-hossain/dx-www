...
impl BuildFrontend {
    ...
    pub fn build_wheel(&self, output_dir: &Path) -> Result<PathBuf> {
        ...
        let wheel_path = build_env.build_wheel(&self.project_dir, output_dir, &self.build_backend)?;
    ...
    pub fn build_sdist(&self, output_dir: &Path) -> Result<PathBuf> {
        ...
        let sdist_path = build_env.build_sdist(&self.project_dir, output_dir, &self.build_backend)?;
    ...
    fn install_requirements(&self, requirements: &[String]) -> Result<()> {
        ...
        if !status.success() {
            return Err(Error::Cache(
                "Failed to install build requirements".to_string(),
            ));
        }
