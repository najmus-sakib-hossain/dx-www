...
fn arb_marker() -> impl Strategy<Value = Marker> {
    (
        "[a-z_][a-z0-9_]{0,20}",
        prop::collection::vec("[a-zA-Z0-9_]+", 0..5),
    )
...
fn arb_test_result() -> impl Strategy<Value = TestResult> {
    (
        ...
        .prop_map(|(id, status, dur_us, stdout, stderr, tb, assertions)| {
            TestResult {
                test_id: id,
                status,
...
fn test_status_is_success() {
    ...
    assert!(TestStatus::Skip { reason: "skipped".into() }.is_success());
    assert!(!TestStatus::Fail.is_success());
    assert!(!TestStatus::Error { message: "error".into() }.is_success());
}

#[test]
fn test_status_is_failure() {
    assert!(!TestStatus::Pass.is_failure());
    assert!(!TestStatus::Skip { reason: "skipped".into() }.is_failure());
    assert!(TestStatus::Fail.is_failure());
    assert!(TestStatus::Error { message: "error".into() }.is_failure());
}
