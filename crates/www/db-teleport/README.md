...
> **"Sub-millisecond database responses through reactive caching"**

**dx-db-teleport** provides reactive database caching with automatic invalidation via Postgres NOTIFY, enabling sub-0.1ms cache access for dx-www applications.

## Performance Targets

| Metric | Target | Description |
|--------|--------|-------------|
| **Cache Access** | < 0.1ms | Sub-millisecond cached query retrieval |
| **Invalidation** | Real-time | Postgres NOTIFY triggers instant cache removal |
...
// Cache is automatically invalidated when Postgres sends NOTIFY
// for tables that the cached query depends on
```
...
// Execute and cache
let result = teleport.execute_and_cache(
    "user_by_id",
...
```rust
// Query depends on "users" table
let query = Query::new("SELECT * FROM users WHERE id = $1")
    .depends_on(&["users"]);

// When "users" table changes, cache is invalidated
teleport.register_query("user_by_id", query);
```
...
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                    Cache Flow                             │   │
│  │  Query → Hash → Cache Lookup → Hit? → Return Binary      │   │
│  │                              → Miss? → Execute → Cache    │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
| Module | Description |
|--------|-------------|
| `cache` | LRU cache with binary storage and TTL |
| `query` | Query representation and dependency tracking |
| `postgres` | Postgres connection pool and NOTIFY listener |
| `error` | Error types for cache and database operations |
...
1. **Cache Consistency** - Cached data matches original query result
2. **Invalidation Correctness** - NOTIFY triggers remove affected entries
3. **Access Latency** - get_cached() returns within 0.1ms target
