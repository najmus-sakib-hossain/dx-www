...
impl TransitionConfig {
    ...
    pub fn new(from_route: u16, to_route: u16, transition_type: TransitionType, duration_ms: u16) -> Self {
        Self {
            from_route,
            to_route,
            transition_type,
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        for i in 0..morph_count.min(MAX_MORPH_PAIRS as u8) as usize {
            let offset = Self::BASE_SIZE + 1 + i * 4;
            if offset + 4 <= bytes.len() {
                morph_pairs[i] = MorphPair::from_bytes(&bytes[offset..offset + 4])?;
            }
        }
