...
impl<'a> UnifiedPipeline<'a> {
    ...
    fn skip_type_annotation(&mut self) {
        ...
        while self.pos < self.source.len() {
            ...
            } else if depth == 0 && (byte == b'=' || byte == b',' || byte == b')' 
                || byte == b'}' || byte == b'\n' || byte == b'{' || byte == b';') {
                break;
            }
            self.pos += 1;
    ...
    fn needs_whitespace(&self) -> bool {
        if self.pos == 0 || self.output.len() == 0 {
            return false;
        }
...
mod tests {
    use super::*;
    use dx_bundle_core::{BundleArena, ArenaOutput};
    
    #[test]
    fn test_import_transform() {
        ...
        UnifiedPipeline::transform(source, &mut output, &imports, 0, &options).unwrap();
        let result = output.as_slice();
        
        assert!(result.windows(7).any(|w| w == b"require"));
    }
    
    #[test]
    fn test_jsx_transform() {
        let source = b"const x = <div>Hello</div>;";
        let arena = BundleArena::new(1024);
        let mut output = ArenaOutput::new(&arena);
        let imports = ImportMap::new();
        let options = TransformOptions::default();
        ...
        assert!(result.windows(18).any(|w| w == b"React.createElement"));
    }
}
