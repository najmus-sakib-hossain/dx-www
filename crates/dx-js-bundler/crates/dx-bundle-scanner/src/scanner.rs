...
use dx_bundle_core::{ScanResult, TypeScriptPattern, SourceSpan};
...
impl<'a> SimdScanner<'a> {
    ...
    pub fn has_typescript(&self) -> bool {
        self.has_pattern(b"interface ") 
            || self.has_pattern(b"type ")
            || self.has_pattern(b": ")
    }
        ...
        while i + 32 <= len {
            ...
            if mask != 0 {
                ...
                while m != 0 {
                    ...
                    if pos + 7 <= len && &self.source[pos..pos+7] == b"export " {
                        if !self.in_string_or_comment(pos) {
                            positions.push(pos as u32);
                        }
                    }
    ...
    unsafe fn find_jsx_avx2(&self, positions: &mut Vec<u32>) {
        ...
        while i + 1 < len {
            if self.source[i] == b'<' {
                ...
                if next.is_ascii_alphabetic() || next == b'/' || next == b'>' {
                    if !self.in_string_or_comment(i) {
                        positions.push(i as u32);
                    }
                }
            }
        ...
        while i + 32 <= len {
            ...
            if i_mask != 0 {
                ...
                while m != 0 {
                    ...
                    if pos + 10 <= len && &self.source[pos..pos+10] == b"interface " {
                        if !self.in_string_or_comment(pos) {
    ...
    fn in_string_or_comment(&self, pos: usize) -> bool {
        ...
        while i < slice.len() {
            if in_comment {
                ...
                if slice[i] == b'"' || slice[i] == b'\'' || slice[i] == b'`' {
                    in_string = true;
                    string_char = slice[i];
...
mod tests {
    ...
    fn test_in_string_filtering() {
        ...
        let mut positions = Vec::new();
        SimdScanner::new(source).find_imports(&mut positions);
        assert_eq!(positions.len(), 0); // Should not find import in string
    }
}
