use self::{
    ...
    toml::TomlFileSource, toml_handler::TomlFileHandler, unknown::UnknownFileHandler,
};
...
impl From<TomlFileSource> for DocumentFileSource {
    fn from(value: TomlFileSource) -> Self {
        Self::Toml(value)
    }
}
...
impl DocumentFileSource {
    ...
    fn try_from_extension(
        ...
        if let Ok(file_source) = TomlFileSource::try_from_extension(extension) {
            return Ok(file_source.into());
        }
    ...
    fn try_from_language_id(language_id: &str) -> Result<Self, FileSourceError> {
        ...
        if let Ok(file_source) = TomlFileSource::try_from_language_id(language_id) {
            return Ok(file_source.into());
        }
...
impl Features {
    ...
    pub(crate) fn get_capabilities(&self, language_hint: DocumentFileSource) -> Capabilities {
        match language_hint {
            ...
            DocumentFileSource::Toml(_) => self.toml.capabilities(),
