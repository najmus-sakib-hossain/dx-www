...
fn compile_rules_with_source_path<P: AsRef<Path>, Q: AsRef<Path>>(
    ...
    println!("   Rules per KB: {}", database.rule_count as f64 / (binary_size as f64 / 1024.0));
...
fn serialize_to_llm(database: &DxRuleDatabase) -> Result<String> {
    ...
    for language in languages {
        ...
        output.push_str(&format!("## {} Rules ({})\n\n", language.prefix().to_uppercase(), rules.len()));
...
pub fn load_compiled_rules<P: AsRef<Path>>(rules_path: P) -> Result<DxRuleDatabase> {
    let binary = fs::read(rules_path.as_ref())
        .context("Failed to read compiled rules file")?;

    // Decompress if needed
    #[cfg(feature = "compression")]
    let binary = {
        use lz4_flex::decompress_size_prepended;
        decompress_size_prepended(&binary)
            .context("Failed to decompress rules")?
    };
