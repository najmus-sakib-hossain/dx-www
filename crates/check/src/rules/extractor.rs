...
use super::builtin;
use super::schema::{
    DxCategory, DxRule, DxRuleDatabase, DxSeverity, Language, RuleSource,
};
use std::path::Path;
...
fn extract_biome_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let biome_js_rules = vec![
        ("noAccumulatingSpread", "Disallow spreading objects that may have large number of properties", DxCategory::Performance),
        ("noAssignInExpressions", "Disallow assignments in expressions", DxCategory::Suspicious),
        ("noAsyncPromiseExecutor", "Disallow async Promise executors", DxCategory::Suspicious),
        ("noCatchAssign", "Disallow reassigning exceptions in catch clauses", DxCategory::Suspicious),
        ("noClassAssign", "Disallow reassigning class members", DxCategory::Suspicious),
        ("noCommentText", "Prevent comments from being inserted as text nodes", DxCategory::Suspicious),
        ...
        ("noControlCharactersInRegex", "Disallow control characters in regex", DxCategory::Suspicious),
        ...
        ("noDuplicateClassMembers", "Disallow duplicate class members", DxCategory::Suspicious),
        ("noDuplicateObjectKeys", "Disallow duplicate keys in object literals", DxCategory::Suspicious),
        ("noDuplicateParameters", "Disallow duplicate parameters", DxCategory::Suspicious),
        ("noEmptyCharacterClassInRegex", "Disallow empty character classes in regex", DxCategory::Suspicious),
        ("noEmptyPattern", "Disallow empty destructuring patterns", DxCategory::Suspicious),
        ("noExtraNonNullAssertion", "Disallow extra non-null assertions", DxCategory::Suspicious),
        ("noFallthroughSwitchClause", "Disallow fallthrough switch cases", DxCategory::Suspicious),
        ("noGlobalObjectCalls", "Disallow calling global objects as functions", DxCategory::Suspicious),
        ("noInnerDeclarations", "Disallow variable or function declarations in nested blocks", DxCategory::Suspicious),
        ("noInvalidConstructorSuper", "Disallow invalid super() calls", DxCategory::Correctness),
        ("noNonoctalDecimalEscape", "Disallow \\8 and \\9 escape sequences", DxCategory::Suspicious),
        ("noPrecisionLoss", "Disallow number literals that lose precision", DxCategory::Correctness),
        ("noPrototypeBuiltins", "Disallow direct use of Object.prototype builtins", DxCategory::Suspicious),
        ("noRedeclare", "Disallow variable redeclaration", DxCategory::Suspicious),
        ("noSelfAssign", "Disallow assignments where both sides are exactly the same", DxCategory::Correctness),
        ("noShadowRestrictedNames", "Disallow shadowing of restricted names", DxCategory::Suspicious),
        ("noSparseArray", "Disallow sparse arrays", DxCategory::Suspicious),
        ("noUnsafeFinally", "Disallow control flow statements in finally blocks", DxCategory::Correctness),
        ("noUnsafeOptionalChaining", "Disallow unsafe optional chaining", DxCategory::Correctness),
        ...
        ("useValidForDirection", "Enforce for loop update clause moving the counter in the right direction", DxCategory::Correctness),
        ("useYield", "Require generator functions to contain yield", DxCategory::Correctness),
    ];
    ...
    let biome_ts_rules = vec![
        ("noExplicitAny", "Disallow explicit any", DxCategory::Types),
        ("noExtraNonNullAssertion", "Disallow extra non-null assertions", DxCategory::Types),
    ...
    for (name, description, category) in biome_ts_rules {
        db.add_rule(
            DxRule::new(
                rule_id,
                Language::TypeScript,
    ...
    let biome_css_rules = vec![
        ...
        ("noInvalidPositionAtImportRule", "Disallow invalid position for @import", DxCategory::Correctness),
    ];
    ...
    let total = 36 + 5 + 3 + 3;
    println!("  ✓ {} biome rules (JS/TS/JSON/CSS)", total);
    rule_id
}
...
fn extract_oxc_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let oxc_rules = vec![
        ("for-direction", "Enforce for loop update clause moving the counter in the right direction", DxCategory::Correctness),
        ("getter-return", "Enforce return statements in getters", DxCategory::Correctness),
        ("no-async-promise-executor", "Disallow async Promise executors", DxCategory::Suspicious),
        ("no-compare-neg-zero", "Disallow comparing against -0", DxCategory::Suspicious),
        ("no-cond-assign", "Disallow assignment operators in conditional expressions", DxCategory::Suspicious),
        ("no-const-assign", "Disallow reassigning const variables", DxCategory::Correctness),
        ("no-constant-binary-expression", "Disallow expressions where the operation doesn't affect the value", DxCategory::Correctness),
        ("no-dupe-else-if", "Disallow duplicate conditions in if-else-if chains", DxCategory::Correctness),
        ("no-duplicate-case", "Disallow duplicate case labels", DxCategory::Suspicious),
        ("no-empty-character-class", "Disallow empty character classes in regex", DxCategory::Suspicious),
        ("no-ex-assign", "Disallow reassigning exceptions in catch clauses", DxCategory::Suspicious),
        ("no-fallthrough", "Disallow fallthrough of case statements", DxCategory::Suspicious),
        ("no-func-assign", "Disallow reassigning function declarations", DxCategory::Suspicious),
        ("no-import-assign", "Disallow assigning to imported bindings", DxCategory::Correctness),
        ("no-inner-declarations", "Disallow variable or function declarations in nested blocks", DxCategory::Suspicious),
        ("no-irregular-whitespace", "Disallow irregular whitespace", DxCategory::Suspicious),
        ("no-self-assign", "Disallow assignments where both sides are exactly the same", DxCategory::Correctness),
        ("no-sparse-arrays", "Disallow sparse arrays", DxCategory::Suspicious),
        ("no-unreachable", "Disallow unreachable code after return, throw, continue, and break", DxCategory::Correctness),
        ("no-unsafe-negation", "Disallow negating the left operand of relational operators", DxCategory::Suspicious),
    ];
...
fn extract_ruff_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let ruff_rules = vec![
        ...
        ("F403", "Star imports used; unable to detect undefined names", DxCategory::Imports),
        ("F404", "Late future import", DxCategory::Imports),
        ("F405", "Name may be undefined, or defined from star imports", DxCategory::Correctness),
        ("F406", "from __future__ imports must occur at the beginning of the file", DxCategory::Imports),
        ...
        ("F507", "% format mismatch of placeholder and argument count", DxCategory::Correctness),
        ...
        ("F525", ".format(...) mixing automatic and manual numbering", DxCategory::Correctness),
        ...
        ("F621", "Too many expressions in star-unpacking assignment", DxCategory::Correctness),
...
fn extract_mago_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let mago_rules = vec![
        ...
        ("no-duplicate-property", "Disallow duplicate class properties", DxCategory::Correctness),
        ("no-empty-block", "Disallow empty blocks", DxCategory::Style),
        ("prefer-null-coalesce", "Prefer null coalesce operator", DxCategory::Style),
        ("no-deprecated-function", "Disallow deprecated functions", DxCategory::Deprecated),
    ];
...
fn extract_go_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let gold_rules = vec![
        ...
        ("varcheck", "Find unused global variables and constants", DxCategory::Correctness),
    ];
...
fn extract_rust_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let clippy_rules = vec![
        ...
        ("clippy::missing_errors_doc", "Missing errors documentation", DxCategory::Documentation),
        ("clippy::missing_panics_doc", "Missing panics documentation", DxCategory::Documentation),
        ("clippy::cognitive_complexity", "Function is too complex", DxCategory::Complexity),
        ...
        ("clippy::match_wildcard_for_single_variants", "Match wildcard for single variants", DxCategory::Style),
    ];
...
fn extract_markdown_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let rumdl_rules = vec![
        ("MD001", "Header levels should only increment by one level at a time", DxCategory::Style),
        ...
        ("MD031", "Fenced code blocks should be surrounded by blank lines", DxCategory::Style),
        ...
        ("MD040", "Fenced code blocks should have a language specified", DxCategory::Style),
        ...
        ("MD044", "Proper names should have the correct capitalization", DxCategory::Style),
...
fn extract_cpp_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let cpp_rules = vec![
        ...
        ("cert-err58-cpp", "Do not throw in constructor of non-local variable", DxCategory::Correctness),
        ...
        ("performance-unnecessary-copy-initialization", "Unnecessary copy initialization", DxCategory::Performance),
        ("readability-const-return-type", "Const return type is unnecessary", DxCategory::Style),
        ("readability-identifier-naming", "Identifier naming convention", DxCategory::Style),
        ("readability-implicit-bool-conversion", "Implicit bool conversion", DxCategory::Style),
    ];
...
fn extract_kotlin_rules(db: &mut DxRuleDatabase, mut rule_id: u16) -> u16 {
    ...
    let ktlint_rules = vec![
        ...
        ("no-consecutive-blank-lines", "Disallow consecutive blank lines", DxCategory::Style),
