//! .dxs File Parser
//!
//! Parses .dxs (DX Serializer) rule files into DxRule structures.
//! Uses a simple line-based parser for the LLM format.
...
/// Parse a single .dxs file
pub fn parse_dxs_file(path: &Path) -> Result<Vec<DxRule>> {
    let content = fs::read_to_string(path)
        .with_context(|| format!("Failed to read .dxs file: {}", path.display()))?;

    parse_dxs_content(&content, path)
}

/// Parse .dxs content from a string
pub fn parse_dxs_content(content: &str, path: &Path) -> Result<Vec<DxRule>> {
        ...
        if let Some(ref section) = current_section {
            if in_multiline {
                if line.starts_with("  ") {
                    ...
                    parse_field_line(trimmed, section, &mut meta, &mut current_rule, 
                                   &mut in_multiline, &mut multiline_field)?;
                }
            } else {
                parse_field_line(trimmed, section, &mut meta, &mut current_rule,
                               &mut in_multiline, &mut multiline_field)?;
    ...
    if in_multiline {
        let value = multiline_value.join("\n");
        set_field(&current_section.unwrap(), &multiline_field, &value, &mut meta, &mut current_rule)?;
    }
...
/// Load all .dxs files from a directory
pub fn load_dxs_directory(dir: &Path) -> Result<DxRuleDatabase> {
    let mut db = DxRuleDatabase::new();

    println!("📂 Loading .dxs files from: {}", dir.display());
    ...
    for entry in entries {
        ...
        if path.extension().and_then(|e| e.to_str()) == Some("dxs") {
            println!("  📄 Parsing: {}", path.file_name().unwrap().to_string_lossy());
            let rules = parse_dxs_file(&path)?;
    ...
    println!("✅ Loaded {} rules from .dxs files\n", db.rule_count);
    Ok(db)
}
...
"#;
        ...
        let path = Path::new("test.dxs");
        let rules = parse_dxs_content(content, path).unwrap();
