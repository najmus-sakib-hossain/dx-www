//! .dxs File Generator
//!
//! Generates human-readable .dxs files from extracted rules.
//! Uses dx-serializer LLM format for maximum readability.

use crate::rules::extractor::*;
use crate::rules::schema::*;
use anyhow::Result;
use std::fs;
use std::path::Path;

/// Generate all .dxs files in the specified directory
pub fn generate_all_dxs_files(output_dir: &Path) -> Result<()> {
    ...
    // Generate one .dxs file per language
    let mut total_rules = 0;
    for (language, rules) in &language_groups {
        let filename = format!("{}-rules.dxs", language_to_prefix(*language));
        let filepath = output_dir.join(&filename);

        generate_dxs_file(&filepath, *language, rules)?;
...
/// Generate a single .dxs file for one language
fn generate_dxs_file(filepath: &Path, language: Language, rules: &[DxRule]) -> Result<()> {
    ...
    // Add each rule
    for rule in rules {
        content.push_str(&rule_to_dxs(rule));
        content.push_str("\n");
    }
...
/// Convert a single rule to .dxs format
fn rule_to_dxs(rule: &DxRule) -> String {
...
mod tests {
    ...
    fn test_rule_to_dxs() {
        ...
        assert!(dxs.contains("@rule"));
        assert!(dxs.contains("name: \"test-rule\""));
        assert!(dxs.contains("category: \"correctness\""));
        assert!(dxs.contains("fixable: true"));
    }
}
