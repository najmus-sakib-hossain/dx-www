...
impl CiContext {
    ...
    fn detect_gitlab() -> Self {
        let mr_iid = env::var("CI_MERGE_REQUEST_IID")
            .ok()
    ...
    fn detect_azure() -> Self {
        let pr_id = env::var("SYSTEM_PULLREQUEST_PULLREQUESTID")
            .ok()
            .and_then(|n| n.parse().ok());
...
impl CiFormatter {
    ...
    pub fn print_summary(&self, files_checked: usize, errors: usize, warnings: usize, duration_ms: u64) {
        match self.context.platform {
            CiPlatform::GitHubActions => {
                ...
                if let Ok(summary_file) = env::var("GITHUB_STEP_SUMMARY") {
                    let summary = format!(
                        ...
                        | Warnings | {} |\n\
                        | Duration | {}ms |\n",
                        files_checked, errors, warnings, duration_ms
                    );
            ...
            CiPlatform::GitLabCi => {
                ...
                println!(
                    "DX Check: {} files, {} errors, {} warnings ({} ms)",
                    files_checked, errors, warnings, duration_ms
                );
            }
