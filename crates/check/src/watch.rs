...
//!
//! Monitors .dxs files and dx config for changes and automatically recompiles.
...
pub fn watch_rules(config: WatchConfig) -> Result<()> {
    ...
    if let Err(e) = compiler::compile_from_dxs(&config.rules_dir, &config.output_dir) {
        ...
        match rx.recv_timeout(Duration::from_millis(100)) {
            Ok(event) => {
                ...
                if matches!(event.kind, EventKind::Modify(_) | EventKind::Create(_)) {
                    // Check if it's a .dxs file or dx config
                    let is_relevant = event.paths.iter().any(|path| {
                        path.extension().and_then(|e| e.to_str()) == Some("dxs")
                    ...
                    if is_relevant {
                        ...
                        match compiler::compile_from_dxs(&config.rules_dir, &config.output_dir) {
                            Ok(compiled) => {
                                println!("✅ Recompiled {} rules ({} KB)", 
                                       compiled.count, compiled.binary_size / 1024);
