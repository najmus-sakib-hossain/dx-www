...
impl ProjectProfile {
    ...
    fn detect_from_package_json(&mut self, pkg: &PackageJson) {
        ...
        // Framework detection - order matters for specificity
        if deps.contains("next") {
            self.frameworks.push(Framework::Next);
        }
        if deps.contains("react") {
            self.frameworks.push(Framework::React);
        }
        if deps.contains("nuxt") {
            self.frameworks.push(Framework::Nuxt);
        }
        if deps.contains("vue") {
            self.frameworks.push(Framework::Vue);
        }
        if deps.contains("@sveltejs/kit") {
            self.frameworks.push(Framework::SvelteKit);
        }
        if deps.contains("svelte") {
            self.frameworks.push(Framework::Svelte);
        }
        if deps.contains("@angular/core") {
            self.frameworks.push(Framework::Angular);
        }
        if deps.contains("solid-js") {
            self.frameworks.push(Framework::Solid);
        }
        if deps.contains("@builder.io/qwik") {
            self.frameworks.push(Framework::Qwik);
        }
        if deps.contains("@remix-run/react") {
            self.frameworks.push(Framework::Remix);
        }
        if deps.contains("astro") {
            self.frameworks.push(Framework::Astro);
        }
        if deps.contains("express") {
            self.frameworks.push(Framework::Express);
        }
        if deps.contains("fastify") {
            self.frameworks.push(Framework::Fastify);
        }
        if deps.contains("hono") {
            self.frameworks.push(Framework::Hono);
        }
        if deps.contains("@nestjs/core") {
            self.frameworks.push(Framework::Nest);
        }
    ...
    pub fn summary(&self) -> String {
        ...
        if !self.frameworks.is_empty() {
            let frameworks: Vec<_> = self.frameworks.iter().map(|f| f.as_str()).collect();
