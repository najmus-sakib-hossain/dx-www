...
use dx_pkg_registry_index::{RegistryIndex, VersionInfo};
...
impl SpeculativePipeline {
    ...
    pub async fn run(&self, dependencies: Vec<ManifestDep>) -> Result<Vec<DownloadedPackage>, PipelineError> {
        ...
        let resolver = tokio::spawn(async move {
            ...
            while let Some((name, constraint)) = queue.pop_front() {
                ...
                if let Some(version) = index.get_version(&name, &constraint) {
                    ...
                    resolve_tx.send(ResolvedPackage {
                        name: name.clone(),
        ...
        let resolved_count = resolver.await.unwrap_or(0);
        downloader.await.ok();
