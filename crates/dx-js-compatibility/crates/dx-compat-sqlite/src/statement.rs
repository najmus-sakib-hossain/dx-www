...
mod tests {
    ...
    fn test_prepared_statement_all() {
        ...
        db.exec("CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)")
            .unwrap();
        db.run("INSERT INTO test (name) VALUES (?)", &[Value::from("Alice")])
            .unwrap();
        db.run("INSERT INTO test (name) VALUES (?)", &[Value::from("Bob")])
            .unwrap();
    ...
    fn test_prepared_statement_get() {
        ...
        db.exec("CREATE TABLE test (id INTEGER PRIMARY KEY, value INTEGER)")
            .unwrap();
        db.run("INSERT INTO test (value) VALUES (?)", &[Value::from(42)])
            .unwrap();
