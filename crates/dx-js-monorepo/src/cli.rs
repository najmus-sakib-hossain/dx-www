...
impl Cli {
    ...
    fn cmd_init(&mut self) -> CliResult {
        ...
        let manifest_path = self.cwd.join(".dx-monorepo").join("workspace.bwm");
        if manifest_path.exists() {
            return CliResult::error(1, "Workspace already initialized");
        }

        // Create workspace directory
        let workspace_dir = self.cwd.join(".dx-monorepo");
    ...
    fn cmd_run(&mut self, task: &str, filter: Option<&str>) -> CliResult {
        ...
        // Load task graph
        let graph_path = self.cwd.join(".dx-monorepo").join("tasks.btg");
    ...
    fn cmd_help(&self) -> CliResult {
        let help = r#"dx-monorepo - Binary-first monorepo management

USAGE:
    dx-monorepo <COMMAND> [OPTIONS]
...
EXAMPLES:
    dx-monorepo init
    dx-monorepo run build
    dx-monorepo run test --filter=@myorg/*
    dx-monorepo affected --base=main
    dx-monorepo ghost
    dx-monorepo watch build
    dx-monorepo cache status
    dx-monorepo cache clear
"#;
...
mod tests {
    ...
    fn test_cli_help() {
        ...
        assert!(result.message.contains("dx-monorepo"));
