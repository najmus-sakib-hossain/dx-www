...
impl WarmCache {
    ...
    pub fn stats(&self) -> CacheStats {
        ...
        let bytes_saved: usize = self.hot.iter()
            .map(|entry| entry.transformed.len())
            .sum();
    ...
    fn load_entry(&self, reader: &mut impl std::io::Read) -> std::io::Result<(ModuleId, CachedTransform)> {
        use std::io::Read;
