use crate::converters::utils::{
    extract_styles, svg_to_astro, svg_to_jsx, svg_to_qwik, svg_to_react_native, svg_to_solid,
    to_pascal_case,
};
...
impl SvgIcon {
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let filename =
            path_ref.file_stem().and_then(|s| s.to_str()).unwrap_or("unknown").to_string();
                        ...
                        if let Ok(attr) = attr {
                            match attr.key.as_ref() {
                                ...
                                b"width" => {
                                    ...
                                    width = w_str.trim_end_matches("px").parse::<u32>().ok();
                                }
                                b"height" => {
                                    let h_str = String::from_utf8_lossy(attr.value.as_ref());
                                    height = h_str.trim_end_matches("px").parse::<u32>().ok();
                                }
                                _ => {}
                            }
                        }
    ...
    pub fn build_collection(icons: &[SvgIcon]) -> Vec<u8> {
        use crate::icon_generated::dx_icon::{
            SvglCollection, SvglCollectionArgs, SvglIcon, SvglIconArgs,
        };
        ...
        let collection = SvglCollection::create(
            &mut builder,
            &SvglCollectionArgs {
