...
impl BinaryCache {
    ...
    pub fn stats(&self) -> CacheStats {
        let entry_count = self.index.read().map(|i| i.len()).unwrap_or(0);
        let total_size = self.current_size.read().map(|s| *s).unwrap_or(0);
    ...
    fn ensure_space(&self, needed: u64) -> std::io::Result<()> {
        ...
        let mut entries: Vec<_> = self.index.read()
...
impl SimpleHasher {
    ...
    fn update(&mut self, data: &[u8]) {
        for byte in data {
            ...
            self.state[(self.pos + 7) % 32] = self.state[(self.pos + 7) % 32].wrapping_mul(31).wrapping_add(*byte);
