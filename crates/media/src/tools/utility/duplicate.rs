...
fn hash_candidates(
    ...
    let files: Vec<PathBuf> = size_groups
        .into_values()
        .flatten()
        .collect();
...
impl SimpleHasher {
    ...
    fn update(&mut self, data: &[u8]) {
        for byte in data {
            ...
            self.state[(idx + 7) % 32] = self.state[(idx + 7) % 32]
                .wrapping_mul(31)
                .wrapping_add(*byte);
            self.state[(idx + 13) % 32] = self.state[(idx + 13) % 32]
                .rotate_left(3)
                .wrapping_add(*byte);
