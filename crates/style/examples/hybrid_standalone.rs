...
fn main() {
    ...
    println!("\n💾 Size Comparison:");
    println!("  Atomic mode: {} IDs × 2 bytes = {} bytes", frequent_ids.len(), frequent_ids.len() * 2);
    println!("  Macro mode:  1 ID × 2 bytes = 2 bytes");
    println!("  Savings: {} bytes ({:.0}% reduction)\n", 
    ...
    println!("├──────────────────┼────────────┼────────────┼──────────┤");
    println!("│ Total Bytes      │ {:>7} B  │ {:>7} B  │ {:>5.1}%  │", 
    ...
    println!("  • Bytes saved: {} bytes ({:.0}% reduction)",
             total_bytes_naive - total_bytes_hybrid,
             ((total_bytes_naive - total_bytes_hybrid) as f64 / total_bytes_naive as f64) * 100.0);
