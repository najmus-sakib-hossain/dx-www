...
fn main() {
    ...
    // Test classes
    let classes = vec!["flex", "items-center", "p-4", "text-white", "bg-blue-500", "rounded-lg"];
    ...
    let combo_ids = vec![ids[0], ids[1], ids[2]];
    ...
    if let Some(combo_id) = is_common_combo(&combo_ids) {
        ...
        println!("\nPayload reduction:");
        println!("  Individual: {} IDs × 2 bytes = {} bytes", combo_ids.len(), combo_ids.len() * 2);
        println!("  Combo: 1 ID × 2 bytes = 2 bytes");
        println!("  Savings: {}%", ((combo_ids.len() * 2 - 2) as f64 / (combo_ids.len() * 2) as f64 * 100.0));
    } else {
    ...
    for (i, &id) in ids.iter().enumerate() {
        ...
        println!("  ID {} → {:?} ({} byte{})", id, enc, enc.len(), if enc.len() == 1 { "" } else { "s" });
    }
    
    println!("\nPayload comparison:");
    println!("  Original: {} bytes (u16 array)", original_size);
    println!("  Varint: {} bytes", varint_size);
    println!("  Savings: {}%", ((original_size - varint_size) as f64 / original_size as f64 * 100.0));
    ...
    println!("  Savings: {}×", string_equivalent.len() as f64 / binary_stream.len() as f64);
    println!();
    ...
    println!("│ Level 1: IDs     │ {:>3} bytes│  {:>3}%    │   1×     │", ids_size, 
    ...
    println!("│ Level 4: Varint  │ {:>3} bytes│  {:>3}%    │   1×     │", varint_size,
    ...
    println!("✅ All optimization levels working!");
}
