...
impl StyleAnalyzer {
    ...
    pub fn scan_file(&mut self, path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for cap in class_regex.captures_iter(&content) {
            if let Some(classes_str) = cap.get(1) {
                let classes: Vec<&str> = classes_str
                    .as_str()
                    .split_whitespace()
                    .collect();
            ...
            for entry in fs::read_dir(dir)? {
                ...
                if path.is_dir() {
                    ...
                    if let Some(ext) = path.extension() {
                        ...
                        if ext == "tsx" || ext == "jsx" || ext == "html" || ext == "ts" || ext == "js" {
    ...
    pub fn generate_combo_code(&self, top_n: usize, min_occurrences: usize) -> String {
        ...
        for pattern in &patterns {
            ...
            code.push_str(&format!(
                "    ({}, \"{}\", {}),  // Used {} times → MACRO\n",
                macro_id, combo, pattern.classes.len(), pattern.count
            ));
            macro_id += 1;
        }
    ...
    pub fn print_report(&self, top_n: usize, min_occurrences: usize) {
        ...
        println!("Top {} patterns (≥{} occurrences):\n", top_n, min_occurrences);
        ...
        let total_macro_ids: usize = patterns.len() * patterns.iter().map(|p| p.count).sum::<usize>();
        let bytes_saved = (total_individual_ids * 2) - (total_macro_ids * 2);
        
        println!("📊 OPTIMIZATION IMPACT:");
        println!("  → Macro entries: {}", patterns.len());
        println!("  → Individual IDs: {} → Macro IDs: {}", total_individual_ids, total_macro_ids);
        println!("  → Bytes saved: {} bytes ({:.1}% reduction)",
                 bytes_saved,
                 (bytes_saved as f64 / (total_individual_ids * 2) as f64) * 100.0);
        println!("\n🎯 STRATEGY:");
        println!("  • Frequent patterns (≥{} uses) → Auto-grouped as MACROs", min_occurrences);
...
mod tests {
    ...
    fn test_scan_html() {
        ...
        let flex_pattern = patterns.iter()
