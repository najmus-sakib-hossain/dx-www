...
use anyhow::Result;
use forge::{
    DaemonConfig, DaemonEvent, ForgeDaemon, LspBridge, WorkerPool, WorkerTask, TaskPriority,
};
...
async fn main() -> Result<()> {
    ...
    worker_pool.queue_with_priority(
        WorkerTask::IndexProject { root: project_root.to_string_lossy().to_string() },
        TaskPriority::High,
    ).await;
    
    worker_pool.queue(WorkerTask::WarmCache { tool: "bundler".to_string() }).await;
    worker_pool.queue(WorkerTask::WarmCache { tool: "style".to_string() }).await;
    worker_pool.queue(WorkerTask::AnalyzePatterns { paths: vec!["src".to_string()] }).await;
    ...
    println!();
    println!("  ```rust");
    println!("  use forge::{{ForgeDaemon, DaemonConfig}};");
    println!();
    println!("  #[tokio::main]");
