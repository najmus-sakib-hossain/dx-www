...
//!
//! Executes dx-serializer to convert .dx files to .dxs binary format
...
pub fn get_binary_path(source_path: &Path, project_root: &Path) -> PathBuf {
    ...
    project_root
        ...
        .join("serializer")
        .join(&hash)
        .join(format!("{}.dxs", filename))
}

/// DX Serializer Tool
///
/// Converts .dx files to .dxs binary format with hashed paths.
...
impl SerializerTool {
    ...
    fn find_dx_files_recursive(dir: &Path, root: &Path, files: &mut Vec<PathBuf>) {
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                Self::find_dx_files_recursive(&path, root, files);
