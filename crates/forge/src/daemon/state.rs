...
impl ToolState {
    ...
    pub fn record_execution(&mut self, success: bool, duration_ms: u64, hits: u64, misses: u64) {
        ...
        let total_duration = self.avg_duration_ms * (self.total_runs - 1) as f64 + duration_ms as f64;
...
impl DaemonStateManager {
    ...
    pub fn record_tool_execution(
        ...
        if let Some(project) = projects.get_mut(project_root) {
            project
                .tool(tool_id.folder_name())
                .record_execution(success, duration_ms, cache_hits, cache_misses);
        }
    ...
    pub fn load(&self) -> Result<()> {
        if self.state_file.exists() {
            ...
            let projects: HashMap<PathBuf, ProjectState> = serde_json::from_str(&content)?;
    ...
    pub fn stats(&self) -> DaemonStateStats {
        ...
        for project in projects.values() {
            for (tool_id, tool) in &project.tools {
                ...
                let entry = tool_stats.entry(tool_id.clone()).or_insert_with(|| ToolState::new(tool_id));
