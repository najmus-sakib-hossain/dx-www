...
impl ForgeDaemon {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.start_time.write() = Some(Instant::now());
        ...
        println!("║  Background Workers: {}                                       ║", self.config.worker_count);
        println!("║  R2 Sync: {}                                                ║", 
            if self.config.enable_r2_sync { "Enabled " } else { "Disabled" });
        println!("╚══════════════════════════════════════════════════════════════╝");
        println!();
        ...
        *self.state.write() = DaemonState::Running;
        ...
        println!("📡 LSP Watcher: {}", if self.config.enable_lsp_watcher { "Active" } else { "Disabled" });
        println!("📁 FS Watcher: {}", if self.config.enable_fs_watcher { "Active" } else { "Disabled" });
        println!();
    ...
    async fn handle_file_change(&self, change: FileChange) {
        ...
        println!(
            ...
            source,
            kind,
            change.path.strip_prefix(&self.config.project_root).unwrap_or(&change.path).display()
        );
