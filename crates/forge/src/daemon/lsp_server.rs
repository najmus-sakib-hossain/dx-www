...
async fn handle_socket(socket: WebSocket, state: LspServerState) {
    ...
    println!("🔗 Extension connected (total: {})", state.connection_count.load(Ordering::SeqCst));
...
fn execute_request(req: LspRequest, state: &LspServerState) -> LspResponse {
    match req {
        ...
        LspRequest::GetStatus => {
            let uptime = state.uptime_start.read()
        ...
        LspRequest::ListTools => {
            ...
            LspResponse::Tools {
                tools: tools.into_iter().map(|t| LspToolInfo {
                    name: t.name,
                    version: t.version,
