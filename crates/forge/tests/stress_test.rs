...
use anyhow::Result;
use dx_forge::{
    create_platform_io, PlatformIO, WriteOp,
    DualWatcher, FileChange,
};
...
async fn stress_test_concurrent_write_operations() -> Result<()> {
    ...
    println!(
        ...
        num_files * writes_per_file, num_files, writes_per_file, elapsed
    );
...
async fn stress_test_batch_operations() -> Result<()> {
    ...
    for i in 0..num_files {
        ...
        let content = format!("Batch content for file {} with some padding data to make it larger", i);
...
async fn stress_test_resource_cleanup() -> Result<()> {
    ...
    let elapsed = start.elapsed();
    println!(
        "Completed {} handle acquire/release cycles in {:?}",
...
async fn stress_test_mixed_operations() -> Result<()> {
    ...
    println!(
        ...
        num_files * operations_per_file, num_files, operations_per_file, elapsed
    );
...
async fn test_fallback_degradation() -> Result<()> {
    ...
    let paths: Vec<PathBuf> = (0..5)
        .map(|i| temp_dir.path().join(format!("batch_{}.txt", i)))
        .collect();
...
async fn test_error_recovery() -> Result<()> {
    use dx_forge::{with_retry, RetryPolicy, ErrorCategory};
    ...
    assert_eq!(attempt_count, max_attempts, "Should have attempted {} times", max_attempts);
    
    // Test retry exhaustion
    let mut exhaustion_count = 0;
    let exhaustion_result = with_retry(
        || {
            exhaustion_count += 1;
