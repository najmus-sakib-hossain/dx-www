...
The platform I/O system provides:
- **Automatic backend selection** - io_uring on Linux, kqueue on macOS, IOCP on Windows
- **Graceful fallback** - Falls back to tokio when native APIs are unavailable
- **Batch operations** - Optimized for high-throughput scenarios
- **File watching** - Platform-native file system monitoring

## Supported Platforms

| Platform | Backend | Minimum Version | Features |
|----------|---------|-----------------|----------|
| Linux | io_uring | Kernel 5.1+ | Batch submit, SQPOLL, IOPOLL |
| macOS | kqueue | 11.0+ | VNode events, timer events |
| Windows | IOCP | 10/Server 2019+ | Overlapped I/O, directory changes |
| Any | Fallback | - | Cross-platform tokio-based I/O |
...
pub struct HandleGuard<'a> {
    // Automatically releases handle when dropped
}
```
...
impl ConfigValidator {
    pub fn validate(config: &ForgeConfig) -> ValidationResult;
...
**Exported Metrics:**
- `files_watched` - Number of files being watched
- `operations_total` - Total I/O operations performed
- `cache_hit_rate` - Cache hit percentage (0-100)
- `errors_total` - Total errors encountered
- `io_latency_p50_us` - 50th percentile latency in microseconds
- `io_latency_p99_us` - 99th percentile latency in microseconds
...
pub struct ShutdownConfig {
    ...
    pub force_after_timeout: bool,   // Default: true
    pub flush_logs: bool,            // Default: true
    pub save_state: bool,            // Default: true
}
```
...
async fn main() -> anyhow::Result<()> {
    ...
    // Write a file
    io.write_all(Path::new("hello.txt"), b"Hello, World!").await?;
    
    // Read it back
    let content = io.read_all(Path::new("hello.txt")).await?;
...
async fn main() -> anyhow::Result<()> {
    ...
    // Batch write multiple files
    let ops = vec![
        WriteOp::new("file1.txt", b"Content 1".to_vec()),
        WriteOp::new("file2.txt", b"Content 2".to_vec()),
        WriteOp::with_sync("important.txt", b"Critical data".to_vec()),
    ];
...
#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let manager = ResourceManager::new(1024);
...
async fn main() {
    ...
    match io.read_all(Path::new("nonexistent.txt")).await {
        Ok(content) => println!("Read {} bytes", content.len()),
        Err(e) => {
            let category = categorize_error(&e);
            match category {
                ErrorCategory::FileSystem => println!("File system error: {}", e),
                ErrorCategory::Timeout => println!("Operation timed out"),
                _ => println!("Error: {}", e),
            }
        }
    }
