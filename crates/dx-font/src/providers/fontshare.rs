...
impl FontProviderTrait for FontshareProvider {
    ...
    async fn get_font_family(&self, font_id: &str) -> Result<FontFamily> {
        ...
        let response: FontshareFont = self.client.get(&url).send().await?.json().await?;
        ...
        Ok(FontFamily {
            ...
            preview_url: Some(format!("https://www.fontshare.com/fonts/{}", response.slug)),
            download_url: Some(format!(
                "https://www.fontshare.com/fonts/{}/download",
                response.slug
            )),
    ...
    async fn health_check(&self) -> Result<bool> {
        let response = self.client.head("https://www.fontshare.com").send().await?;
