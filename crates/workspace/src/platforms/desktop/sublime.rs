...
impl DesktopGenerator for SublimeGenerator {
    ...
    fn exists(&self, project_dir: &Path) -> bool {
        ...
        if let Ok(entries) = fs::read_dir(project_dir) {
            for entry in entries.flatten() {
                if entry.path().extension().map_or(false, |ext| ext == "sublime-project") {
                    return true;
                }
            }
        }
    ...
    fn clean(&self, project_dir: &Path) -> Result<()> {
        ...
        if let Ok(entries) = fs::read_dir(project_dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.extension().map_or(false, |ext| ext == "sublime-project") {
                    fs::remove_file(&path).map_err(|e| crate::Error::io(&path, e))?;
                }
                if path.extension().map_or(false, |ext| ext == "sublime-workspace") {
                    fs::remove_file(&path).map_err(|e| crate::Error::io(&path, e))?;
                }
            }
        }
