...
dx-test-executor    → Parallel work-stealing executor
...
2. Test Compilation (cached)
   ├─ Parse test files
   ├─ Generate bytecode
   └─ Write to binary layout

3. Test Execution
   ├─ Memory-map layout
   ├─ Execute bytecode (parallel)
   └─ Collect results
```
...
```javascript
// Import from dx-test
import { test, expect } from 'dx-test';
