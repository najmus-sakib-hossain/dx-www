...
fn prop_opcode_uniqueness() {
    ...
    for opcode in all_opcodes {
        ...
        // Each opcode value must fit in a single byte (u8 guarantees this)
        assert!(value <= 255, "Opcode {:?} value {} exceeds u8 max", opcode, value);
        
        // Each opcode must have a unique value
        assert!(
            seen_values.insert(value),
            "Opcode {:?} has duplicate value {}", opcode, value
        );
    }
    
    // Verify we have the expected number of opcodes
    assert_eq!(all_opcodes.len(), seen_values.len(),
...
fn prop_opcode_roundtrip() {
    ...
    for opcode in HbtpOpcode::all() {
        ...
        assert_eq!(
            recovered, Some(*opcode),
            "Opcode {:?} (value {}) should roundtrip correctly", opcode, value
        );
    }
}
...
fn prop_header_size_invariant() {
    ...
    assert_eq!(
        size_of::<HbtpHeader>(), 8,
        "HbtpHeader must be exactly 8 bytes, got {}", size_of::<HbtpHeader>()
    );
    
    // Also verify the constant matches
    assert_eq!(
        HbtpHeader::SIZE, 8,
        "HbtpHeader::SIZE must be 8"
    );
    
    assert_eq!(
        size_of::<HbtpHeader>(), HbtpHeader::SIZE,
...
fn prop_header_parsing_boundary() {
    ...
    let buffer_7 = [0u8; 7];
    assert!(HbtpHeader::from_bytes(&buffer_7).is_none(),
        "7 bytes should fail");
    
    // Exactly 8 bytes - should succeed
    let buffer_8 = [0u8; 8];
    assert!(HbtpHeader::from_bytes(&buffer_8).is_some(),
        "8 bytes should succeed");
    
    // Empty buffer - should fail
    let empty: [u8; 0] = [];
    assert!(HbtpHeader::from_bytes(&empty).is_none(),
        "empty buffer should fail");
}
...
fn prop_flag_composition_combinations() {
    ...
    for i in 0u8..16 {
        ...
        let mut flags = HbtpFlags::empty();
        if compressed { flags |= HbtpFlags::COMPRESSED; }
        if encrypted { flags |= HbtpFlags::ENCRYPTED; }
        if expects_response { flags |= HbtpFlags::EXPECTS_RESPONSE; }
        if is_final { flags |= HbtpFlags::FINAL; }
...
fn dummy_handler(_header: &HbtpHeader, _payload: &[u8]) -> Result<Vec<u8>, dx_reactor::protocol::HbtpError> {
    Ok(vec![])
}
...
proptest! {
    ...
    fn prop_teleport_roundtrip_u64(value in any::<u64>()) {
        ...
        prop_assert!(recovered.is_some(), "Should be able to read u64");
    ...
    fn prop_teleport_roundtrip_i32(value in any::<i32>()) {
        ...
        prop_assert!(recovered.is_some(), "Should be able to read i32");
    ...
    fn prop_teleport_roundtrip_f64(value in any::<f64>()) {
        ...
        prop_assert!(recovered.is_some(), "Should be able to read f64");
    ...
    fn prop_teleport_roundtrip_multiple(
        ...
        prop_assert!(r1.is_some() && r2.is_some() && r3.is_some(),
    ...
    fn prop_teleport_roundtrip_string(s in "[a-zA-Z0-9 ]{0,100}") {
        ...
        prop_assert!(recovered.is_some(), "Should be able to read string");
...
fn prop_teleport_roundtrip_slice() {
    ...
    assert!(recovered.is_some(), "Should be able to read slice");
...
use dx_reactor::middleware::{TimingMiddleware, Request as MwRequest, Response as MwResponse, Middleware as MwTrait};
...
fn prop_timing_header_without_start_time() {
    ...
    assert!(res.header("X-Response-Time").is_none(), 
