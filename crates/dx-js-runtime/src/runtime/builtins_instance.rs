...
impl ArrayPrototype {
    ...
    pub fn for_each(&self, array: &[Value], callback: Box<dyn Fn(&Value, usize)>) {
    ...
    pub fn every(&self, array: &[Value], callback: Box<dyn Fn(&Value, usize) -> bool>) -> bool {
        array.iter().enumerate().all(|(i, val)| callback(val, i))
    }

    /// Array.prototype.some(callback)
    pub fn some(&self, array: &[Value], callback: Box<dyn Fn(&Value, usize) -> bool>) -> bool {
...
mod tests {
    ...
    fn test_number_to_fixed() {
        ...
        assert_eq!(proto.to_fixed(3.14159, 2), "3.14");
    }
}
