...
#[test]
fn test_io_queue() {
    use dx_js_runtime::io::{IOQueue, IORequest, IOOperation};
...
fn test_distributed_runtime() {
    use dx_js_runtime::distributed::{DistributedRuntime, NodeInfo, NodeCapabilities};
...
fn test_task_scheduler() {
    use dx_js_runtime::distributed::{Task, TaskScheduler, DistributedRuntime, NodeInfo, NodeCapabilities};
    use std::net::SocketAddr;
...
fn test_node_selection() {
    ...
    runtime.register_node(NodeInfo {
        ...
        capabilities: NodeCapabilities { cpu_cores: 2, memory_gb: 4, gpu_available: false },
        load: 0.9,
    });
    ...
    runtime.register_node(NodeInfo {
        ...
        capabilities: NodeCapabilities { cpu_cores: 8, memory_gb: 16, gpu_available: true },
        load: 0.2,
    });
    
    let required = NodeCapabilities { cpu_cores: 4, memory_gb: 8, gpu_available: false };
...
fn test_deployment_config() {
    use dx_js_runtime::deploy::{DeploymentConfig, OptimizationLevel};
...
fn test_task_requirements() {
    use dx_js_runtime::distributed::{Task, NodeCapabilities};
    
    let task = Task::new(100, vec![5, 6, 7, 8])
        .with_requirements(NodeCapabilities {
            cpu_cores: 4,
            memory_gb: 8,
            gpu_available: true,
        });
