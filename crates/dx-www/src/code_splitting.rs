...
impl ClassificationResult {
    ...
    pub fn chunk_count(&self) -> usize {
        let mut count = 0;
        if !self.critical.is_empty() { count += 1; }
        if !self.interactive.is_empty() { count += 1; }
        if !self.submission.is_empty() { count += 1; }
        if !self.navigation.is_empty() { count += 1; }
        if !self.rare.is_empty() { count += 1; }
        count
    }
