...
static CODE_REGIONS: OnceLock<Arc<RwLock<HashMap<PathBuf, Vec<GeneratedRegion>>>>> =
    OnceLock::new();
...
pub fn mark_code_region_as_dx_generated(
    ...
    regions.entry(file.to_path_buf()).or_insert_with(Vec::new).push(region);

    tracing::debug!(
        "🏷️  Marked lines {}-{} in {:?} as generated by {}",
        start_line,
        end_line,
        file,
        generator_tool
